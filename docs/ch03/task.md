...
> 基于已批准的 spec.md + plan.md。任务有序，每步留绿（`python -m micodeagent` 可启动 / `pytest` 通过 / `ruff check` 无告警）。验证一律「先跑命令看输出，再下结论」。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `src/micodeagent/llm/__init__.py` | 新增 ToolCall/ToolResult/ToolDefinition/ROLE_TOOL；扩展 Message/StreamEvent；Provider.stream 加 tools 参数 |
| 修改 | `src/micodeagent/llm/anthropic_provider.py` | 注入 tools、stream 解析 tool_use blocks、tool_use/tool_result 回灌 |
| 修改 | `src/micodeagent/llm/openai_provider.py` | 注入 tools、按 index 拼 tool_calls、assistant.tool_calls/tool 消息回灌 |
| 新建 | `src/micodeagent/tool/__init__.py` | Tool Protocol、Result、Registry、new_default_registry、DEFAULT_TIMEOUT、_truncate |
| 新建 | `src/micodeagent/tool/{read_file,write_file,edit_file,bash,glob_tool,grep_tool}.py` | 6 个核心工具 |
| 新建 | `tests/test_tool.py` | 注册中心 + 各工具单测 |
| 新建 | `src/micodeagent/agent/__init__.py` | Agent、Event、ToolEvent、Phase、run（单轮闭环） |
| 新建 | `tests/test_agent.py` | fake provider 驱动单轮闭环（AC8/AC9） |
| 修改 | `src/micodeagent/conversation.py` | add_assistant_with_tool_calls、add_tool_results |
| 修改 | `src/micodeagent/prompt.py` | SYSTEM_PROMPT 增 Agent 角色与工具约定 |
| 修改 | `src/micodeagent/tui/{app,stream,view}.py` | 接入 Agent.run、工具事件渲染、工具行/执行指示 |
| 修改 | `src/micodeagent/cli.py` | 构造 new_default_registry 注入 MiCodeAgentApp |

---

## T1: 扩展 llm 协议无关类型**文件：** `src/micodeagent/llm/__init__.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.llm import ToolCall, ToolResult, ToolDefinition, ROLE_TOOL, Message, StreamEvent; print(Message(role='tool').tool_results)"` 输出 `[]`；`ruff check src/micodeagent/llm/__init__.py` 无告警。

## T2: tool 包骨架（Tool Protocol、Result、Registry、_truncate）**文件：** `src/micodeagent/tool/__init__.py`
**依赖：** T1
**步骤：**
...
**验证：** `python -c "from micodeagent.tool import Tool, Result, Registry, DEFAULT_TIMEOUT; print(Registry().definitions())"` 输出 `[]`；`ruff check src/micodeagent/tool/` 无告警。

## T3: read_file 工具**文件：** `src/micodeagent/tool/read_file.py`
**依赖：** T2
**步骤：**
...
**验证：** `python -c "import asyncio; from micodeagent.tool.read_file import ReadFileTool; print(asyncio.run(ReadFileTool().execute('{\"path\":\"pyproject.toml\"}')).content[:80])"` 出现行号；读不存在文件得 `is_error=True`（T9 后补单测）。

## T4: write_file 工具**文件：** `src/micodeagent/tool/write_file.py`
**依赖：** T2
**步骤：**
...
## T5: edit_file 工具**文件：** `src/micodeagent/tool/edit_file.py`
**依赖：** T2
**步骤：**
...
## T6: bash 工具**文件：** `src/micodeagent/tool/bash.py`
**依赖：** T2
**步骤：**
1. `class BashTool`。
...
## T7: glob 工具**文件：** `src/micodeagent/tool/glob_tool.py`
**依赖：** T2
**步骤：**
1. `class GlobTool`。
...
**验证：** `ruff check`；T9 后单测 `**/*.py` 能命中 `src/micodeagent/` 下文件。

## T8: grep 工具**文件：** `src/micodeagent/tool/grep_tool.py`
**依赖：** T2
**步骤：**
1. `class GrepTool`。
...
## T9: new_default_registry 与 tool 单测**文件：** `src/micodeagent/tool/__init__.py`、`tests/test_tool.py`
**依赖：** T3–T8
**步骤：**
...
## T10: Provider.stream 加 tools 参数（注入定义，暂不解析）**文件：** `src/micodeagent/llm/__init__.py`、`src/micodeagent/llm/anthropic_provider.py`、`src/micodeagent/llm/openai_provider.py`、`src/micodeagent/tui/stream.py`
**依赖：** T1
**步骤：**
...
**验证：** `python -m micodeagent` 发一条纯文本仍正常（工具定义已随请求发送，模型未必调用）；`ruff check src/micodeagent/llm/` 无告警。

## T11: anthropic 适配器解析工具调用 + 回灌**文件：** `src/micodeagent/llm/anthropic_provider.py`
**依赖：** T10
**步骤：**
...
**验证：** `python -m micodeagent` 启动正常；`ruff check src/micodeagent/llm/anthropic_provider.py` 无告警。

## T12: openai 适配器解析工具调用 + 回灌**文件：** `src/micodeagent/llm/openai_provider.py`
**依赖：** T10
**步骤：**
...
**验证：** `python -m micodeagent` 启动正常；`ruff check src/micodeagent/llm/openai_provider.py` 无告警。

## T13: conversation 扩展**文件：** `src/micodeagent/conversation.py`、`tests/test_conversation.py`
**依赖：** T1
**步骤：**
...
## T14: agent 单轮闭环**文件：** `src/micodeagent/agent/__init__.py`、`tests/test_agent.py`
**依赖：** T9, T11, T12, T13
**步骤：**
...
## T15: prompt 系统提示词扩展**文件：** `src/micodeagent/prompt.py`
**依赖：** 无
**步骤：**
1. 扩写 `SYSTEM_PROMPT`：说明 MiCodeAgent 是能使用工具的 Agent，可读写改文件、执行命令、查找/搜索代码；需要信息或操作时调用相应工具，拿到结果后给出简洁答复。

**验证：** `ruff check src/micodeagent/prompt.py`；`pytest` 不回归。

## T16: tui 接入 agent + 工具行渲染**文件：** `src/micodeagent/tui/app.py`、`src/micodeagent/tui/stream.py`、`src/micodeagent/tui/view.py`
**依赖：** T14, T15
**步骤：**
1. `app.py`：`MiCodeAgentApp.__init__(self, providers, version, registry)` 存 `self._registry: Registry`；新增成员 `self._cur_tool: ToolDisplay | None = None`（小 dataclass：`name: str, args: str`）。
...
**验证：** `python -m micodeagent` 启动正常；`ruff check src/micodeagent/tui/` 无告警。

## T17: cli 接线**文件：** `src/micodeagent/cli.py`
**依赖：** T16
**步骤：**
1. `from micodeagent.tool import new_default_registry`；构造 `registry = new_default_registry()`；`MiCodeAgentApp(cfg.providers, __version__, registry).run()`。

**验证：** `python -m micodeagent` 在合法配置下能启动 TUI 并进入对话。

## T18: 全量验证与端到端冒烟**文件：** 无（验证）
**依赖：** T1–T17
**步骤：**
1. `ruff format --check .`；`ruff check .`；`pytest -v`（可选 `mypy src/micodeagent`）。
2. 用当前 `.micodeagent/config.yaml`（openai 兼容端点）跑：问「读 docs/python/ch03/spec.md 并用一句话总结」→ 观察工具行 `● read_file(...)` + 结果摘要 + 最终答复（AC8/AC11）。
