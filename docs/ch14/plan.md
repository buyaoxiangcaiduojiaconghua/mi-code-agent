...
新建 `src/micodeagent/worktree/` 子包,集中放 Manager、Worktree、WorktreeSession、Slug 校验、创建后设置、自动清理、过期清理。其余包按以下方式接入:

- **`micodeagent.tool`**:新增 `ctx.py`(with_cwd / cwd_from_ctx / resolve_path);改造 6 个核心工具用 resolve_path
- **`micodeagent.subagent`**:`Definition` 加 `isolation` 字段,`parser.py` 解析 `isolation:` frontmatter
- **`micodeagent.agent`**:`AgentTool.execute` 加 `_execute_with_worktree` 分支,启动时通过 ctx 注入 cwd
- **`micodeagent.command`**:新增 `builtin_worktree.py`,提供 `/worktree` 一级命令与子命令(create/list/enter/exit/remove)
- **`micodeagent.tui`**:在 App 字段加 `worktree_mgr: worktree.Manager`、`active_cwd: str`;主 Agent 每次 Run 前用 `with_cwd(active_cwd)` 包住
- **`src/micodeagent/__main__.py` / `cli.py`**:`Manager(root)` 落在 `subagent_catalog = load_subagent_catalog(root)` 之后;失败降级为 None(可选);把 Manager 传给 `MiCodeAgentApp` 和 `AgentTool`
- **`.gitignore`**:追加 `.micodeagent/worktrees/` 与 `.micodeagent/worktree_session.json`
...
class Manager:
    ...
    # repo_root: 绝对路径
    # worktree_dir: <repo_root>/.micodeagent/worktrees
    # session_file: <repo_root>/.micodeagent/worktree_session.json
...
## 模块设计### `micodeagent.worktree`(新子包)**职责:** Worktree 完整生命周期管理 + Slug 校验 + 后台清理。
...
### `micodeagent.tool` 改造**职责:** 增加 ctx cwd 传递机制,改造 6 个工具用 resolve_path / 子进程 cwd 参数。
...
### `micodeagent.subagent` 改造**职责:** Definition 加 isolation 字段;parser 解析。
...
### `micodeagent.agent` 改造**职责:** AgentTool 增加 worktree 分支,接受 Manager。
...
- 新增 `agent_worktree.py`:
  ...
  - 直接 `from micodeagent.worktree import random_agent_name`(worktree 包不依赖 agent,无导入循环)

### `micodeagent.command` 新增**职责:** `/worktree` 一级命令 + 子命令解析。
...
### `micodeagent.tui` 改造**职责:** 持有 Manager 引用,把 active_cwd 注入主 Agent ctx。

**改动:**

- `app.py` `MiCodeAgentApp` 加属性 `worktree_mgr: worktree.Manager | None`、`active_cwd: str = ""`(空表示进程 cwd)
- `MiCodeAgentApp.__init__` 接收 `worktree_mgr`(或通过依赖注入)
...
- 启动时(`MiCodeAgentApp.__init__` 内)若 Manager 的 `current_session()` 非 None,把 `self.active_cwd = session.worktree_path`

### `src/micodeagent/cli.py` / `__main__.py` 改造
...
src/micodeagent/cli.py / __main__.py      — 接入
...
| Worktree 目录位置                           | `.micodeagent/worktrees/<flat_slug>/`                 | README 既定方案;仓库内 + .gitignore 不追踪                                                 |
| 嵌套 slug`/` 处理                           | 替换为`+`(flat_slug)做文件系统/分支名             | Git 分支的`/` 是命名空间分隔符,会导致 `worktree-team/alice` 与 `worktree-team` 的 D/F 冲突 |
| Manager 构造失败处理                        | 抛异常,micodeagent 降级 worktree_mgr=None             | 不阻塞 micodeagent 启动;后续 isolation:worktree 调用回错误信息                                 |
...
| 后台清理触发时机                            | micodeagent 启动时跑一次,asyncio.create_task 后台执行 | 不阻塞主流程;ch11 已有 session.clean_expired 同样做法                                      |
