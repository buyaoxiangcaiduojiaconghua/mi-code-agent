...
- [ ] src/micodeagent/worktree 子包存在且可导入(验证:`python -c "from micodeagent import worktree"`)
...
- [ ] `manager.create("alice", "HEAD", manual=True)` 在 `.micodeagent/worktrees/alice/` 下落地 + 分支 `worktree-alice`(验证:`pytest tests/test_worktree_create.py`)
- [ ] `manager.create("team/alice", ...)` 落地 `.micodeagent/worktrees/team+alice/` + 分支 `worktree-team+alice`(验证:同上)
...
- [ ] 创建后设置 A——`.micodeagent/settings.local.yaml` 被复制到 Worktree(验证:同上,需在测试 fixture 准备文件)
...
- [ ] `manager.enter` 持久化 session 到 `.micodeagent/worktree_session.json`(验证:同上)
...
- [ ] `agent.AgentTool` 含 `worktree_mgr: worktree.Manager | None` 字段;`__init__` 签名末尾接收 worktree_mgr(验证:`python -c "from micodeagent.agent import AgentTool"`)
...
- [ ] `command.WorktreeSummary` 与 `WorktreeAccessor` Protocol 存在(验证:`python -c "from micodeagent.command.ui import WorktreeAccessor, WorktreeSummary"`)
...
- [ ] `MiCodeAgentApp` 含 `worktree_mgr: worktree.Manager | None` 与 `active_cwd: str` 字段(验证:`python -c "from micodeagent.tui.app import MiCodeAgentApp"`)
- [ ] `MiCodeAgentApp.__init__` 接收 `worktree_mgr` 参数;启动时若 `manager.current_session()` 非 None,设 `active_cwd=session.worktree_path`(验证:`pytest tests/test_tui_app.py -k worktree`)
- [ ] 主 Agent Run 前用 `with with_cwd(...):` 包住——可通过 mock provider 断言 tool 调用收到的 cwd(验证:同上)
- [ ] worktree_adapter 实现 WorktreeAccessor 协议(验证:`python -c "from micodeagent.tui.worktree_adapter import WorktreeAdapter"`)

### cli 接入

- [ ] src/micodeagent/cli.py 构造 Manager,失败 stderr 警告 + 降级(验证:启动 micodeagent + 在非 git 目录测试)
- [ ] AgentTool 构造末尾追加 worktree_mgr(验证:同上)
- [ ] MiCodeAgentApp 构造接收 worktree_mgr(验证:同上)
- [ ] 启动时异步跑 sweep_stale(验证:`grep -n sweep_stale src/micodeagent/cli.py`)
- [ ] .gitignore 追加 `.micodeagent/worktrees/` 与 `.micodeagent/worktree_session.json`(验证:`git check-ignore .micodeagent/worktrees/test`)
...
- [ ] worktree 包 + subagent 包 + agent 包 + command 包 + tui 包之间无导入循环(验证:`python -c "import micodeagent.tui.app, micodeagent.agent.agent_tool, micodeagent.command.builtins, micodeagent.worktree"`)

## 编译与测试

- [ ] 项目可启动:`python -m micodeagent --help`(或正常进 TUI)
- [ ] 所有单元测试通过:`pytest`
- [ ] lint 通过:`ruff check`(可选 `ruff format --check`)

## 端到端场景(tmux 实跑)

每个场景在 tmux 内启动一个 micodeagent 实例完成,验证可视化行为。

**通用预置:**
- 当前目录 `cd /Users/codemelo/micodeagent`
...
.micodeagent/agents/worktree-writer.md
----------------------------------
...
**步骤:**
- [ ] tmux 启动:`tmux new-session -d -s ch14 -x 200 -y 50 "python -m micodeagent"`
...
- [ ] tool_result 中末尾含 `[Worktree 保留: .micodeagent/worktrees/agent-a... ,分支 worktree-agent-a...]`(因为有未提交修改,auto_cleanup 保留)
- [ ] **主目录** `cat scratch_ch14.txt` 仍为 `MAIN`(验证主目录未被改)
- [ ] **Worktree 副本** `cat .micodeagent/worktrees/agent-a*/scratch_ch14.txt` 为 `SUBAGENT`
- [ ] tmux 截屏断言:`tmux capture-pane -p -t ch14 | grep -i "worktree"`
- [ ] 清理:`rm scratch_ch14.txt`,删除残留 worktree:在 micodeagent 内 `/worktree remove agent-a... --discard`(或 `git worktree remove --force` 手动清)
...
- [ ] `ls .micodeagent/worktrees/` 不存在与本次任务对应的 `agent-a*` 目录(已被 auto_cleanup 删除)
- [ ] tmux kill-session
...
- [ ] scrollback 显示 `Worktree 已创建: .micodeagent/worktrees/demo-feature (分支 worktree-demo-feature)`
- [ ] 输入:`/worktree list`
- [ ] scrollback 显示一行含 `demo-feature` 的列表项,标记 `[manual]`(`manual=True`)
- [ ] tmux 外验证:`ls .micodeagent/worktrees/demo-feature/` 含正常 micodeagent 仓库内容;`git -C .micodeagent/worktrees/demo-feature branch` 显示在 `worktree-demo-feature`
- [ ] 清理:输入 `/worktree remove demo-feature --discard`
- [ ] 验证 `.micodeagent/worktrees/demo-feature` 已不存在
- [ ] tmux kill-session

### 场景 4:`/worktree exit` 变更保护**预置:** 同场景 3 创建好 `demo-feature`。

**步骤:**
- [ ] 手动写一个修改:`echo "modified" > .micodeagent/worktrees/demo-feature/test.txt`
- [ ] tmux 启动 micodeagent
...
- [ ] 在 tmux 外:`echo "in-worktree-only" > .micodeagent/worktrees/cwd-test/probe.txt`(主目录无 probe.txt)
...
- [ ] scrollback 显示错误,含「invalid」或「拒绝」(不创建 `.micodeagent/etc/` 或类似)
- [ ] 输入:`/worktree create ..`
- [ ] 同样错误
