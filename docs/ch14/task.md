...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/worktree/__init__.py` | 公开导出 Manager / validate_slug / 错误类型 |
| 新建 | `src/micodeagent/worktree/slug.py` | validate_slug + flat_slug |
| 新建 | `tests/test_worktree_slug.py` | Slug 校验单测 |
| 新建 | `src/micodeagent/worktree/session.py` | WorktreeSession + JSON 原子持久化 |
| 新建 | `src/micodeagent/worktree/git.py` | _run_git + _has_worktree_changes + _resolve_head_sha_from_fs |
| 新建 | `tests/test_worktree_git.py` | git helper 单测 |
| 新建 | `src/micodeagent/worktree/manager.py` | Manager 类型 + 构造 + list/get/current_session |
| 新建 | `src/micodeagent/worktree/create.py` | create + 快速恢复 + post-creation setup A/B/C/D |
| 新建 | `tests/test_worktree_create.py` | create + setup 单测 |
| 新建 | `src/micodeagent/worktree/lifecycle.py` | enter / exit / remove / auto_cleanup |
| 新建 | `tests/test_worktree_lifecycle.py` | 生命周期单测 |
| 新建 | `src/micodeagent/worktree/sweep.py` | sweep_stale + 三层过滤 |
| 新建 | `tests/test_worktree_sweep.py` | sweep_stale 单测 |
| 新建 | `tests/test_worktree_manager.py` | Manager 构造 + session 持久化测试 |
| 新建 | `src/micodeagent/tool/ctx.py` | with_cwd / cwd_from_ctx / resolve_path |
| 新建 | `tests/test_tool_ctx.py` | resolve_path 单测 |
| 修改 | `src/micodeagent/tool/read_file.py` | 用 resolve_path 解析 path |
| 修改 | `src/micodeagent/tool/write_file.py` | 用 resolve_path 解析 path |
| 修改 | `src/micodeagent/tool/edit_file.py` | 用 resolve_path 解析 path |
| 修改 | `src/micodeagent/tool/glob.py` | 用 resolve_path 解析 root |
| 修改 | `src/micodeagent/tool/grep.py` | 用 resolve_path 解析 path |
| 修改 | `src/micodeagent/tool/bash.py` | 子进程 cwd=resolve_path("") |
| 修改 | `src/micodeagent/subagent/definition.py` | Definition 加 isolation 字段 |
| 修改 | `src/micodeagent/subagent/parser.py` | 解析 isolation: frontmatter |
| 修改 | `tests/test_subagent_parser.py` | 增加 isolation 单测 |
| 新建 | `src/micodeagent/agent/agent_worktree.py` | _execute_with_worktree + build_worktree_notice |
| 修改 | `src/micodeagent/agent/agent_tool.py` | 增加 worktree_mgr 字段 + isolation 分支 |
| 新建 | `tests/test_agent_worktree.py` | _execute_with_worktree 单测(用 stub Manager) |
| 修改 | `src/micodeagent/command/ui.py` | 加 WorktreeAccessor 协议 + WorktreeSummary |
| 新建 | `src/micodeagent/command/builtin_worktree.py` | /worktree handler + 子命令解析 |
| 修改 | `src/micodeagent/command/builtins.py` | 注册 /worktree |
| 修改 | `tests/test_command_builtins.py` | 加 worktree 注册测试 |
| 新建 | `src/micodeagent/tui/worktree_adapter.py` | 实现 WorktreeAccessor 适配 worktree.Manager |
| 修改 | `src/micodeagent/tui/app.py` | worktree_mgr / active_cwd 字段 + 注入 ctx |
| 修改 | `src/micodeagent/cli.py` | 构造 Manager + 注入 AgentTool / App + sweep_stale |
| 修改 | `.gitignore` | 追加 .micodeagent/worktrees/ + worktree_session.json |

## T1: Slug 校验**文件:** `src/micodeagent/worktree/slug.py` + `tests/test_worktree_slug.py`
**依赖:** 无
**步骤:**
1. 创建子包 `micodeagent.worktree`,加 `__init__.py`(暂时空导出,后续 T 步骤补)
...
## T2: WorktreeSession 持久化**文件:** `src/micodeagent/worktree/session.py`
**依赖:** T1
**步骤:**
...
## T3: Git helper**文件:** `src/micodeagent/worktree/git.py` + `tests/test_worktree_git.py`
**依赖:** 无
**步骤:**
...
## T4: Manager 构造**文件:** `src/micodeagent/worktree/manager.py` + `tests/test_worktree_manager.py`
**依赖:** T2, T3
**步骤:**
...
## T5: create + 快速恢复 + 创建后设置**文件:** `src/micodeagent/worktree/create.py` + `tests/test_worktree_create.py`
**依赖:** T4
**步骤:**
...
2. 实现 `async def _perform_post_creation_setup(repo_root, wt_path, symlink_dirs)`:四个子函数:
   - `copy_local_configs(repo_root, wt_path)`:对 `.micodeagent/config.yaml` / `.micodeagent/settings.local.yaml`,若主仓存在且 Worktree 不存在,`shutil.copy`
...
## T6: enter / exit / remove / auto_cleanup**文件:** `src/micodeagent/worktree/lifecycle.py` + `tests/test_worktree_lifecycle.py`
**依赖:** T5
**步骤:**
...
## T7: sweep_stale**文件:** `src/micodeagent/worktree/sweep.py` + `tests/test_worktree_sweep.py`
**依赖:** T6
**步骤:**
...
## T8: tool ctx**文件:** `src/micodeagent/tool/ctx.py` + `tests/test_tool_ctx.py`
**依赖:** 无(并行 T1-T7)
**步骤:**
1. 在 `src/micodeagent/tool/ctx.py` 定义 `_ctx_cwd: ContextVar[str | None] = ContextVar("cwd", default=None)`
...
## T9: 改造 6 个核心工具**文件:** `src/micodeagent/tool/{bash,read_file,write_file,edit_file,glob,grep}.py`
**依赖:** T8
**步骤:**
...
## T10: subagent.Definition.isolation**文件:** `src/micodeagent/subagent/{definition,parser}.py` + `tests/test_subagent_parser.py`
**依赖:** 无
**步骤:**
...
## T11: _execute_with_worktree**文件:** `src/micodeagent/agent/agent_worktree.py` + `tests/test_agent_worktree.py`
**依赖:** T6, T8, T10
**步骤:**
1. 新建 `agent_worktree.py`,顶部 `from micodeagent.worktree import Manager, random_agent_name`(worktree 包不依赖 agent,无导入循环)
...
## T12: AgentTool 接入 isolation 分支**文件:** `src/micodeagent/agent/agent_tool.py`
**依赖:** T11
**步骤:**
...
5. 改 `src/micodeagent/cli.py` 的 `AgentTool` 构造调用,传入 `worktree_mgr`

**验证:** `pytest tests/test_agent_tool.py tests/test_agent_worktree.py -v`

## T13: command 包加 WorktreeAccessor + /worktree handler**文件:** `src/micodeagent/command/ui.py` + `src/micodeagent/command/builtin_worktree.py` + `src/micodeagent/command/builtins.py` + `tests/test_command_builtins.py`
**依赖:** T6
**步骤:**
...
## T14: TUI 适配 + 注入 ctx**文件:** `src/micodeagent/tui/worktree_adapter.py` + `src/micodeagent/tui/app.py`
**依赖:** T11, T13
**步骤:**
...
2. `app.py`:`MiCodeAgentApp` 加属性 `worktree_mgr: worktree.Manager | None`、`active_cwd: str = ""`(空表示进程 cwd)
3. `MiCodeAgentApp.__init__` 接收 `worktree_mgr`;构造时若 `manager.current_session()` 非 None,设 `self.active_cwd = session.worktree_path`
...
**验证:** `python -m micodeagent` 可启动;`/worktree create x` + `/worktree enter x` + Read file(相对路径) 在 worktree 内成功

## T15: 主 cli 接入**文件:** `src/micodeagent/cli.py` + `.gitignore`
**依赖:** T4-T14 全部
**步骤:**
...
3. `MiCodeAgentApp` 构造新增 `worktree_mgr=worktree_mgr`
4. `.gitignore` 追加:
   ```
   # ch14: Worktree 隔离副本(仅供 SubAgent 与手动管理使用)
   .micodeagent/worktrees/
   .micodeagent/worktree_session.json
   ```

**验证:** `python -m micodeagent` 可启动、`pytest` 全过、`ruff check` 通过
...
2. 准备项目级自定义 Agent `.micodeagent/agents/worktree-writer.md`(详见 checklist 场景 1)
3. tmux 启动 `python -m micodeagent`,跑 checklist 端到端场景
4. 通过即标记 T16 完成
