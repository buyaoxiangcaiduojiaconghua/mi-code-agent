...
需要的是**空间维度**的隔离:同一仓库同时挂多个工作目录、共享版本库、各自一个分支。这就是 Git Worktree (Git 2.5+) 的能力。本章在 micodeagent 中封装一层 Worktree 管理逻辑,把这块拼图补给 SubAgent,让后台 / 并行场景安全可用。

micodeagent 现有相关基础设施:
- ch13 SubAgent 已支持 frontmatter (`src/micodeagent/subagent/parser.py`),解析 `name/description/tools/disallowed_tools/model/max_turns/permission_mode/background` 等字段
- ch13 `agent.AgentTool.execute` 已是子 Agent 启动入口,本章在此处插桩 isolation 分支
- ch08 文件读缓存以绝对路径作为 key
- ch10 `src/micodeagent/command` 已有 slash 命令注册系统
- `.gitignore` 已忽略 `.micodeagent/sessions/` 等子目录,本章扩展把 `.micodeagent/worktrees/` 也忽略
...
- **G3**: Worktree 目录统一落在仓库内不被追踪的位置 `.micodeagent/worktrees/<flat_slug>/`,分支名前缀 `worktree-<flat_slug>`,嵌套 slug 的 `/` 替换为 `+` 避免 Git D/F 冲突
- **G4**: 创建后做四类环境初始化——A 复制本地配置 (`.micodeagent/config.yaml` / `.micodeagent/settings.local.yaml`)、B 配置子目录的 git hooks (`core.hooksPath` 不自动继承)、C 软链 `node_modules` / `.venv` / `vendor` 等大目录、D 按项目根 `.worktreeinclude` 复制被忽略但运行需要的文件;均为 best-effort,失败只警告不中断创建
...
- **G11**: `WorktreeSession` 持久化到 `.micodeagent/worktree_session.json`,micodeagent 启动时读取并校验目录仍存在;退出时清空文件而不是删文件,确保下次启动不误恢复
...
- **F4**: `worktree.Manager` 内部字段——`repo_root`(绝对路径)、`worktree_dir`(`<repo_root>/.micodeagent/worktrees`)、`session_file`(`<repo_root>/.micodeagent/worktree_session.json`)、`lock: asyncio.Lock`、`active: dict[str, Worktree]`、`current_session: WorktreeSession | None`
- **F5**: `worktree.Manager(repo_root: str)` 构造时(或工厂函数 `Manager.create(repo_root)`)——
  - 校验 `repo_root` 是 git 仓库根目录(`git rev-parse --show-toplevel` 输出与之等);失败抛异常,micodeagent 启动允许降级到「Worktree 功能未启用」
...
- **F6**: `Manager.create(name: str, base_ref: str, manual: bool) -> Worktree`(async)——
  ...
  - 8. 加入 `active`,返回
- **F7**: 创建后设置 A——复制本地配置文件,从 `<repo_root>/.micodeagent/config.yaml` 与 `<repo_root>/.micodeagent/settings.local.yaml` 复制到 Worktree 同位置(目标目录已存在跳过,文件不存在跳过)
...
### explicit cwd 工具改造- **F16**: 在 `micodeagent.tool` 包定义 ctx key 与帮助函数——
...
- **F31**: micodeagent 启动时(`Manager.__init__` 内),读 `session_file` 反序列化;若文件内容为 `null` 或空,`current_session=None`;若 `worktree_path` 不存在,清空文件并 `current_session=None`(stderr 警告 "session worktree gone, cleared")
- **F32**: `--resume` (micodeagent 现有恢复入口)读到已有 session 时,把 `active_cwd` 设置到 `session.worktree_path`,主 Agent 后续工具调用都按 explicit cwd 走
...
- **F34**: micodeagent 启动时跑一次 `asyncio.create_task(manager.sweep_stale(now - timedelta(hours=24)))`(异步、后台执行),不阻塞启动

### .gitignore 更新- **F35**: 在项目根 `.gitignore` 追加 `.micodeagent/worktrees/` 与 `.micodeagent/worktree_session.json` 两行;micodeagent 启动时若发现 `.gitignore` 不含这两行,**只警告不修改**(尊重用户配置)
...
- **N4**: `os.chdir` 在 micodeagent 进程内只出现在 `Manager.exit` 兜底调用;其他地方一律用 explicit cwd
- **N5**: Worktree session 文件被破坏(非法 JSON)启动时只警告并清空,不阻断 micodeagent 启动
- **N6**: 与 ch04~ch13 既有测试零破坏——`pytest` 全绿
- **N7**: 中文友好——错误消息与命令输出全部中文(对齐 micodeagent 其他模块风格)
...
- Windows 平台特殊支持(symlink 行为在 Windows 上不保证;本期 micodeagent 以 macOS / Linux 为主)
- 跨 micodeagent 进程实例的 Worktree 共享(同一仓库同一时刻只支持一个 micodeagent 实例操作 worktree session)
...
- **AC2**: `manager.create("alice", "HEAD", manual=True)` 在 `.micodeagent/worktrees/alice/` 下落地 Worktree,分支为 `worktree-alice`
- **AC3**: `manager.create("team/alice", "HEAD", manual=True)` 在 `.micodeagent/worktrees/team+alice/` 下落地,分支 `worktree-team+alice`
- **AC4**: 已存在 worktree 目录时再调 create 走快速恢复——不调 `git worktree add`,毫秒级返回(单测可断言 git 子进程未启动)
- **AC5**: 创建后设置 A——主仓库存在 `.micodeagent/settings.local.yaml` 时,Worktree 内同位置出现该文件
...
- **AC20**: `WorktreeSession` 持久化到 `.micodeagent/worktree_session.json`,启动时读取;指向的 Worktree 目录被外部删除后,启动时清空 session 并 stderr 警告
- **AC21**: 项目可启动 (`python -m micodeagent`)、所有单元测试通过 (`pytest`)、lint 通过 (`ruff check`)
- **AC22**: tmux 实跑——`python -m micodeagent` 启动 + 触发 `isolation:worktree` 子 Agent 改文件 + 验证主目录 `server.py`(若改的是 `server.py`)未变,Worktree 副本里 `server.py` 已变;Worktree 留盘 / 自动清理符合预期
