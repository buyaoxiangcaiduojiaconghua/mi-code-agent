...
micodeagent 现有相关基础设施:
- 已有的 `task.Manager` 已支持后台任务管理 + `send_message` 续派 + `AgentNameRegistry` (`by_name` 字段已是 name → id 映射);本章扩展为多 Team 寻址
...
- 已有的 `worktree.Manager` 已支持嵌套 slug(`team/alice` → `.micodeagent/worktrees/team+alice/`),本章直接复用做队员 worktree(slug 形式 `team-<team_name>/<member>`)
- 已有的 session 持久化(`.micodeagent/sessions/<id>/conversation.jsonl`)按对话粒度落盘;本章给每个队员单独申请一个 session,队员 stop 不删 session,SendMessage 续派时通过 session 反序列化 Conversation
- 已有的 `micodeagent.command` slash 命令系统,本章新增 `/team` 系列
...
- **G1**: 提供 `team.Team` 与 `team.Manager`——Team 封装小组生命周期(name、lead_agent_id、members、config_path);Manager 在单 micodeagent 进程内管理多个 Team(典型场景同时只有一个活跃 Team)
- **G2**: 提供 `TeamCreate` 工具——主 Agent 调用即创建 Team、调 `detect_backend` 确定后端、写 `~/.micodeagent/teams/<sanitized_name>/config.json`、把 Lead 注册成第一个成员;同名团队自动后缀 `-2` / `-3` 避免冲突
...
- **G9**: 消息分普通文本与结构化两类,统一走 SendMessage 以 `type` 字段分流;`text` 直接拼进队友下一轮 prompt;`shutdown_request` / `shutdown_response` 做优雅退出协商;`plan_approval_response` 是 Lead 对队员计划的审批回复(只允许 Lead 发)。结构化消息带一个 `request_id`,应答时原样带回好和请求对上号,表态字段 `approve` 用可空布尔(`Optional[bool]`,默认 `None`)区分「没表态」与「明确拒绝」
...
- **G18**: 与既有功能协同——主 Agent 平时(未 TeamCreate)看到的工具列表不变;协作工具仅在 Team 上下文出现;已有的后台任务 / AdoptRunning / SendMessage 续派路径保留,Team 队员的续派复用同一套底层 `task.Manager`

## 功能需求

### Team 数据结构与 Manager

- **F1**: `team.Team` 字段——`name`(原始名)、`sanitized_name`(经 `sanitize` 处理后用于路径)、`description`(团队描述,随 config.json 落盘)、`lead_agent_id`、`members: list[TeammateInfo]`、`config_dir`(`<home_dir>/.micodeagent/teams/<sanitized_name>/`)、`config_path`(`<config_dir>/config.json`)、`created_at: datetime`、`backend: BackendType`
...
- **F4**: `team.Manager(home_dir: Path, wt_mgr, task_mgr, reg) -> Manager`——校验 `<home_dir>/.micodeagent/teams/` 可写;扫描该目录还原 `teams` dict(每个子目录读一次 `config.json`,跳过解析失败的并 stderr 警告)
...
- **F15**: `micodeagent.team.backend.tmux.TmuxBackend` 实现 `Backend` Protocol
  - `spawn`:`tmux split-window -h -P -F "#{pane_id}" -- <cmd>`(横向 split,-P 打印 pane id,-F 指定格式);`cmd` 为 `python -m micodeagent --team-member --team <team_name> --member <member_name> --agent-id <agent_id> --session-dir <session_dir> --worktree <worktree_path> [--agent-type <type>] [--model <model>] [--plan-mode]`
...
- **F17**: `micodeagent.team.backend.iterm2.Iterm2Backend` 实现 `Backend` Protocol
...
- **F18**: `micodeagent.team.backend.inprocess.InProcessBackend` 实现 `Backend` Protocol
...
- **F19a**: `python -m micodeagent --team-member` 在 Pane 后端被 spawn 的 micodeagent 子进程**不启动 TUI**(不构造 Textual App),而是跑一个自治协程(`src/micodeagent/cli/team_member.py` 的 `run_team_member`):
...
- **F25**: 当 `team_name` 非空,`Agent.execute` 走 Team 分支:
  ...
  9. **若是 Pane 后端**,在 `backend.spawn` 之前把 `initial_prompt` 作为 `text` 消息(`from=lead`)预写入 alice 的 mailbox(F13);in-process 后端不需要,`initial_prompt` 直接作为 `task.Manager.launch` 的 task 参数
...
- **F37**: 注册时机——`Agent` 工具 spawn 队员时(F25 step 10);`AgentTool` 的 `name` 参数非空时(已有功能,本章统一这套 registry,替换 `task.Manager.by_name` 的内部 dict)
- **F38**: 命名冲突——后注册的覆盖前注册的(README 称「弱引用,后启动覆盖前面的弱引用」)

### 队员系统提示词附录

- **F39**: 在子 Agent 的 system_prompt 后追加(若 spawn 进 Team)以下文本(无变量):
  ```
...
- **F41a**: Lead 侧不通过 ctx hook 自动读 mailbox(Lead 没有 `TeammateContext`),而是由 TUI 在 `on_mount` 启动后台 asyncio task `consume_lead_mail`(实现于 `src/micodeagent/tui/tasks.py`):
...
- **F46**: SendMessage 检测到目标 agent_id 已 stop 且为 in-process 队员(`task.BackgroundTask.status` 不是 `Running`):
  ...
  2. 调 `await task.Manager.send_message(parent_ctx, name, message)` 复用已有续派接口
...
- **F47**: Pane 后端队员的续写——SendMessage 写邮箱后,目标 pane 内的 micodeagent 实例下一轮 Loop 自然读到消息;若 pane 已死(`tmux list-panes` 查不到 `pane_id`),报错让 Lead 决定是否重新 spawn
...
- **F49**: 队员在 plan 模式下生成 Plan 后(通过常规 LLM 推理),用 `SendMessage(to="lead", type="text", content="<plan text>")` 发给 Lead——本期不强制结构化 Plan 类型(Lead 自行识别)
- **F50**: Lead 用 `SendMessage(to="<member>", type="plan_approval_response", request_id="<队员计划请求的 request_id>", approve=True|False, content="<驳回时的修改意见>")` 回复
...
- **F52**: 提供 `coordinator.is_enabled() -> bool` 函数:
  ...
  ```
  `feature_has` 通过 `micodeagent.config` 读 `features.coordinator_mode` 字段;`env_truthy` 接受 `"1"` / `"true"` / `"yes"`(大小写不敏感)
...
- **F55**: Coordinator 系统提示词追加在 system_prompt 末尾,核心是"四阶段 + 派完不许自己干"纪律。最终文案见 `src/micodeagent/coordinator/coordinator.py:SYSTEM_PROMPT_SUFFIX`,关键约束:
...
- **F63**: `~/.micodeagent/teams/<sanitized_name>/config.json` 结构:
  ```json
  {
    "name": "...",
    "sanitized_name": "...",
    ...
    "members": [
      {
        ...
        "agent_type": "worker",
        "model": "",
        "worktree_path": "/abs/path/.micodeagent/worktrees/team-foo+alice",
        "branch": "worktree-team-foo+alice",
        "backend_type": "tmux",
        "pane_id": "%5",
        "is_active": null,
        "plan_mode_required": false,
        "session_dir": "/abs/path/.micodeagent/sessions/<id>"
      }
    ]
  }
  ```
...
- **F64**: micodeagent 启动时(`team.Manager` 构造)扫描所有 Team 目录:
...
- **F65**: 队员 session 沿用已有的 session 持久化机制,路径 `<project_root>/.micodeagent/sessions/<id>/conversation.jsonl`;Team 删除时一并删除
...
- **N1**: 主 Agent 平时(未 TeamCreate)看到的工具列表保持稳定——`TeamCreate` / `TeamDelete` 总是可见;`Agent` 工具的 `team_name` 参数对模型可见但仅在调用时校验
...
- **N7**: 中文友好——错误消息、TUI 输出、coordinator 提示词全部中文(对齐 micodeagent 其他模块风格);代码注释中文
- **N8**: Coordinator Mode 一旦启用,Lead 不可在运行时解锁(避免 LLM 被注入后自行解锁);取消的唯一方式是退出 micodeagent 重启
- **N9**: 权限沙箱(`src/micodeagent/permission/sandbox.py`)允许写入项目根**之外**的 `/tmp` 与 macOS 真实路径 `/private/tmp` 作为系统临时目录白名单。理由:工具脚本和队员经常需要 `/tmp` 做中转文件,严格限定在项目根内会导致大量正常用法被沙箱误杀。这一开放对 file-class 工具(read_file / write_file / edit_file)生效;bash 走 exec-class 权限,本来就不受沙箱约束

## 不做的事

- 跨 micodeagent 进程的 Team 共享(同一仓库同一时刻只支持一个 micodeagent 实例操作活跃 Team)
- 跨机器分布式 Team
...
- **AC1**: `team.Manager(...)` 在 `~/.micodeagent/teams/` 不存在时自动创建;已有时正确扫描子目录还原 `teams` dict
- **AC2**: `await manager.create("refactor auth", "")` 把 `"refactor auth"` sanitize 为 `"refactor-auth"`,在 `~/.micodeagent/teams/refactor-auth/config.json` 落地,`backend` 字段反映 `detect_backend` 结果
...
- **AC7**: `Agent` 工具带 `team_name="<existing>"` 时,在 `.micodeagent/worktrees/team-<sanitized>+<member>/` 落地 Worktree、调对应 backend.spawn 并在 `team.members` 里出现该成员;不带 `team_name` 时维持原行为
...
- **AC23**: tmux 后端 spawn 后,`tmux list-panes` 看到新 pane,pane 内 micodeagent 实例启动并连接到该 Team
...
- **AC27**: 项目能正常启动 `python -m micodeagent`;`ruff check src/` 通过;`pytest` 全部通过
- **AC28**: tmux 实跑(端到端):
  - 步骤 1:在 tmux 会话内启动 `micodeagent`
  - 步骤 2:输入 prompt 让主 Agent 调 `TeamCreate(team_name="demo")`,看到状态栏出现 team 标识,`~/.micodeagent/teams/demo/config.json` 落地
  ...
  - 步骤 4:观察 tmux 新增 pane,pane 内出现 micodeagent 子实例;`.micodeagent/worktrees/team-demo+alice/` 目录创建;`/tmp/test_alice.txt` 文件创建,内容 `hello`
...
- **AC29**: in-process 后端实跑(端到端,不依赖 tmux):
  - 步骤 1:`unset TMUX TERM_PROGRAM`,启动 `micodeagent`(自动 fallback in-process)
...
- **AC30**: Coordinator Mode 实跑——`MEWCODE_COORDINATOR_MODE=1` 启动 micodeagent,主 Agent 的 `write_file` 工具调用被拒绝(is_error=True);`bash git merge` 调用允许
```
