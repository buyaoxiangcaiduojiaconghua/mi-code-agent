...
- [ ] `team.Manager` 可被实例化:`team.Manager(home, root, wt_mgr, task_mgr, name_reg)` 返回非 None(验证:`python -c "from micodeagent.team import Manager"`、跑单测)
- [ ] `await team.Manager.create("demo", "")` 在 `~/.micodeagent/teams/demo/config.json` 落地(验证:运行单测后检查文件存在)
- [ ] `await team.Manager.create("foo bar/baz", "")` sanitize 后路径为 `~/.micodeagent/teams/foo-bar-baz/`(验证:单测)
...
- [ ] `Agent` 工具不带 `team_name` 时走原有路径,行为不变(验证:`pytest tests/test_agent_tool.py` 全过)
- [ ] `Agent` 工具带 `team_name="demo"` 时调 `team_hook.spawn_teammate`(验证:单测 mock team_hook,断言被调用)
- [ ] `spawn_teammate` 创建 worktree 路径为 `.micodeagent/worktrees/team-demo+alice`(验证:单测/集成测试)
...
- [ ] 队员 idle 后 Lead mailbox 收到一条正文带 `[idle] <name>` 的文本消息(验证:单测/集成)
...
- [ ] Pane 后端的 `python -m micodeagent --team-member` 子进程**不启动 TUI**(不构造 Textual App),跑 `run_team_member` 自治协程——读 mailbox → run_to_completion → 通知 Lead idle → stdin Wake 等下一轮(验证:tmux 实跑看 alice pane 显示纯文本日志流而非 Textual TUI 框)
...
- [ ] `/team list` 输出含 `~/.micodeagent/teams/` 下所有 Team(验证:TUI 实跑)
...
- macOS / Linux
- tmux 已安装
- 当前不在 micodeagent 进程内,准备开新 tmux 会话

步骤:
- [ ] `tmux new-session -s worktree-test` 进入新 tmux 会话
- [ ] `cd /path/to/micodeagent && uv sync`(预装依赖,加快冷启动)
- [ ] `uv run python -m micodeagent`(或装好的 `micodeagent` 入口)启动 TUI;启动消息显示一切正常,无本章相关 error
- [ ] 在 TUI 输入:「创建一个名为 demo 的团队」
  - 预期:Agent 调 `TeamCreate(team_name="demo")`;返回 `{"team_name":"demo","backend":"tmux","config_path":"..."}`
  - 验证:`ls ~/.micodeagent/teams/demo/config.json` 存在;`cat config.json` 中 `backend` 字段为 `tmux`
- [ ] 在 TUI 输入:「派 alice 用 general-purpose 角色,在 worktree 里跑 `echo hello > /tmp/test_alice.txt && pwd > /tmp/test_alice_pwd.txt`」
  ...
  - 验证 c:`ls /path/to/micodeagent/.micodeagent/worktrees/team-demo+alice/` 目录存在
  - 验证 d:等待 30 秒,`cat /tmp/test_alice.txt` 内容为 `hello`
  - 验证 e:`cat /tmp/test_alice_pwd.txt` 内容为 worktree 路径(`.../team-demo+alice`)
  - 验证 f:`cat ~/.micodeagent/teams/demo/config.json` 中 `members` 数组含 alice,`backend_type="tmux"`,`pane_id` 非空
  - 验证 g:`~/.micodeagent/teams/demo/mailbox/<alice_agent_id>.json` 中应已含一条 `from=lead` 的 text 消息——Pane 后端的 initial_prompt 预写入证据
...
- [ ] 在 TUI 输入:「给 alice 发消息,让她再写一行 world 到 /tmp/test_alice.txt」
  - 预期:Agent 调 `SendMessage(to="alice", content="...")`
...
- [ ] 等待 alice 任务自然结束(或在 TUI 输入 `/team kill alice` 终止)
  - 验证 a:`cat ~/.micodeagent/teams/demo/config.json` 中 alice 的 `is_active` 为 `false`(跨进程 reload 修复——alice 子进程的 `set_member_active(False)` 必须真的反映到 disk;早期 bug 是静默 no-op)
  - 验证 b:Lead 的 mailbox(`cat ~/.micodeagent/teams/demo/mailbox/lead.json`)含一条正文带 `[idle]` 的文本消息,且 1-2 秒后该消息 `read=true`(watcher 已消费)
  - 验证 c:Lead 屏幕**不需要用户输入**自动出现 `● [team-update] 队员发来新消息...` 文本块 + 紧接的 Synthesis 回复(自动唤醒)
- [ ] 在 TUI 输入 `/team delete demo --force`
  - 验证 a:`ls ~/.micodeagent/teams/` 无 `demo` 目录
  - 验证 b:`ls /path/to/micodeagent/.micodeagent/worktrees/` 无 `team-demo+alice`
...
步骤:
- [ ] 启动 `uv run python -m micodeagent`(同会话已 unset 上述变量)
- [ ] 在 TUI 输入:「创建 inproc 团队」
  - 验证:`cat ~/.micodeagent/teams/inproc/config.json` 中 `backend` 为 `in-process`
...
环境准备:
- `.micodeagent/config.yaml` 加 `features:\n  coordinator_mode: true`(snake_case)
- 设环境变量 `MEWCODE_COORDINATOR_MODE=1`

步骤:
- [ ] `MEWCODE_COORDINATOR_MODE=1 uv run python -m micodeagent`
...
- [ ] 在 TUI 输入:「跑 `git status`」
  ...
  - 验证:输出含 git 状态信息
- [ ] 在 TUI 输入:「派几个队员探索 micodeagent 的 src/micodeagent/agent 和 src/micodeagent/team」
...
步骤:
- [ ] 准备一个角色定义 `~/.micodeagent/agents/planner.md`,frontmatter 含 `permission_mode: plan`,body 简述「先制定计划」
- [ ] 启动 micodeagent,创建 team `plan-test`
...
- [ ] 在 TUI 输入:「批准 planner 的计划」
  - 预期:Lead 调 `SendMessage(to="planner", type="plan_approval_response", request_id="<planner 计划请求的 request_id>", approve=True)`
  - 验证:planner 收到后切换权限模式,继续执行计划

## 失败回归

- [ ] micodeagent 启动时 `~/.micodeagent/teams/` 不存在,自动创建,不报错
- [ ] `~/.micodeagent/teams/<somename>/config.json` 内容损坏时,启动只 stderr 警告,跳过该 Team
