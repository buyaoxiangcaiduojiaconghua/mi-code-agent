...
本章引入 `micodeagent.team` 顶层包,把 SubAgent 的「子 Agent」扩展为「Team 队员」。整体分四层:
...
Lead 仍是 `tui.MiCodeAgentApp.main_agent`——本期 Lead 没有独立类型,通过 `coordinator.is_enabled()` 在启动时收窄其工具集即可。

依赖方向(单向):
```

tui  ──→  agent  ──→  team  ──→  team/{backend,mailbox,registry,tasks,tools}
                       └──→  worktree、task、session、subagent
...
@dataclass
class Message:
    from_: str                       # 属性名避开保留字,json key "from"
    text: str
    timestamp: str = ""              # ISO 8601 字符串
    read: bool = False
    # 结构化消息用这三个字段,普通文本消息留空
    type: MessageType = MessageType.TEXT
    request_id: str = ""             # json key "requestId"
...
```python
# src/micodeagent/coordinator/__init__.py
...
**职责:** Team / TeammateInfo / Manager 数据结构与持久化,跨子包的协调入口。
...
- 新增 `agent.TeamHook` Protocol:
  ```python   class TeamHook(Protocol):       # spawn_teammate 让 Agent 工具委托给 Team Manager 处理 team_name 分支。       # 返回 final_text(立即返回 task_id JSON 描述)。       async def spawn_teammate(self, req: "TeamSpawnRequest") -> str: ...       # is_teammate_context 判断当前上下文是否在某队员的执行上下文中(用于拦截嵌套 spawn)。       def is_teammate_context(self, ctx) -> tuple[str, str, bool]: ...   ```
- `AgentTool` 持一个 `team_hook: TeamHook | None` 字段(可选,None 时降级为原有行为)
...
- `MiCodeAgentApp` 新增字段 `team_mgr: team.Manager`
- 注入 `/team` 系列 slash 命令(`src/micodeagent/command/builtin_team.py`)
...
3. mkdir ~/.micodeagent/teams/demo/
4. mkdir ~/.micodeagent/teams/demo/mailbox/
5. 写 config.json(原子)
...
6. 申请 session_dir(util 函数,沿用已有的 session 格式)
7. 构造 SpawnRequest
...
```
LLM 调 SendMessage(to="alice", content="hello")
  ↓
...
`agent.Agent` 已有 system_reminders 注入机制(已有的系统提示词 / 权限系统的 plan reminder 走同一通道);新增一种 reminder 来源即可。
...
await mailbox.write(lead_agent_id, Message(from_="<name>", type="text", content="[idle] <name> (reason: available)"))
...
| 决策点                                                | 选择                                                                         | 理由                                                                                                       |
| ----------------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Team 包归属                                           | `micodeagent.team` 顶层包                                                        | 与`subagent`、`worktree` 平级,职责清晰                                                                     |
...
| Lead 表示                                             | 不引入独立类型,Lead =`MiCodeAgentApp.main_agent`                                 | 收窄改动;Coordinator Mode 在工具集层面区分                                                                 |
...
| AgentNameRegistry 归属                                | 独立`team.registry` 子包,`task.Manager` 委托                                 | 解耦;消除`task.Manager._by_name` 的局部状态                                                                |
| `task.Manager` 改造                                   | 加`on_task_done` 回调,Team 注册                                              | 依赖反转,避免 task 包反向依赖 team                                                                         |
| Team 持久化原子性                                     | `<file>.tmp` + `os.replace`                                                  | 与已有的 worktree session、session 持久化一致;Python`os.replace` 跨平台原子                                |
| Worktree 命名                                         | `team-<sanitized_team>/<member>`(嵌套 slug,`/` → `+`)                       | 复用已有的嵌套 slug 能力;不污染顶层 worktree 命名空间                                                      |
| Member session_dir                                    | 沿用已有的`<root>/.micodeagent/sessions/<id>/` 格式                              | 复用`session.Writer`,无需新机制;Team 删除时一并清理                                                        |
...
| Team 删除时 Worktree 处理                             | 调`wt_mgr.remove(name, discard_changes=True)`,失败只警告                     | 与 Worktree 退出语义一致;`force=True` 才放行,无 force 时有活跃成员就拒删,有变更也保留(自动 cleanup 已处理) |
...
| 子进程模型                                            | `python -m micodeagent --team-member` + `asyncio.create_subprocess_exec`         | 标准 Python 启动方式,跨平台                                                                                |
