...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/team/__init__.py` | 包导出 |
| 新建 | `src/micodeagent/team/types.py` | Team / TeammateInfo / BackendType 等类型 |
| 新建 | `src/micodeagent/team/persistence.py` | sanitize、原子写、reload_from_disk_locked |
| 新建 | `src/micodeagent/team/manager.py` | Manager.create/get/delete/add_member/set_member_active/remove_member |
| 新建 | `tests/test_team_manager.py` | Manager 单测 |
| 新建 | `src/micodeagent/team/spawn.py` | spawn_teammate 主流程 |
| 新建 | `tests/test_team_spawn.py` | spawn 单测(in-process 路径) |
| 新建 | `src/micodeagent/team/feature.py` | FORK_TEAMMATE flag 读取 |
| 新建 | `src/micodeagent/team/mailbox/__init__.py` | Box.read/write/mark_read + Message 类型 |
| 新建 | `src/micodeagent/team/mailbox/lock.py` | 文件锁机制 |
| 新建 | `src/micodeagent/team/mailbox/message.py` | Message / MessageType |
| 新建 | `tests/test_team_mailbox.py` | 并发与 stale 锁测试 |
| 新建 | `src/micodeagent/team/registry/__init__.py` | AgentNameRegistry |
| 新建 | `tests/test_team_registry.py` | 注册/解析/反查测试 |
| 新建 | `src/micodeagent/team/tasks/__init__.py` | Task / Store / Filter / Patch |
| 新建 | `tests/test_team_tasks.py` | CRUD + 依赖关系测试 |
| 新建 | `src/micodeagent/team/backend/__init__.py` | Backend Protocol + SpawnRequest + new_backend |
| 新建 | `src/micodeagent/team/backend/detect.py` | detect() |
| 新建 | `tests/test_team_backend_detect.py` | 检测逻辑测试 |
| 新建 | `src/micodeagent/team/backend/tmux.py` | Tmux Backend |
| 新建 | `tests/test_team_backend_tmux.py` | tmux 命令构造测试 |
| 新建 | `src/micodeagent/team/backend/iterm2.py` | iTerm2 Backend |
| 新建 | `tests/test_team_backend_iterm2.py` | iterm2 命令构造测试 |
| 新建 | `src/micodeagent/team/backend/inprocess.py` | InProcess Backend |
| 新建 | `tests/test_team_backend_inprocess.py` | in-process spawn 集成测试 |
| 新建 | `src/micodeagent/team/tools/team_create.py` | TeamCreate 工具 |
| 新建 | `src/micodeagent/team/tools/team_delete.py` | TeamDelete 工具 |
| 新建 | `src/micodeagent/team/tools/task_create.py` | TaskCreate 工具 |
| 新建 | `src/micodeagent/team/tools/task_get.py` | TaskGet 工具 |
| 新建 | `src/micodeagent/team/tools/task_list.py` | TaskList 工具 |
| 新建 | `src/micodeagent/team/tools/task_update.py` | TaskUpdate 工具 |
| 新建 | `src/micodeagent/team/tools/send_message.py` | SendMessage 工具 |
| 新建 | `src/micodeagent/team/tools/teammate_filter.py` | 队员专属工具白名单 |
| 新建 | `tests/test_team_tools.py` | 工具单测 |
| 新建 | `src/micodeagent/coordinator/__init__.py` | is_enabled/allowed_tools/system_prompt_suffix |
| 新建 | `tests/test_coordinator.py` | 双锁测试 |
| 新建 | `src/micodeagent/agent/team_hook.py` | TeamHook Protocol + TeammateContext |
| 修改 | `src/micodeagent/agent/agent_tool.py` | 增加 team_name 参数 + TeamHook 委托 + with_team_hook 构造选项 |
| 新建 | `src/micodeagent/agent/team_mailbox.py` | Loop 头部注入 incoming-messages reminder |
| 修改 | `src/micodeagent/task/manager.py` | 加 on_task_done 回调;改用 registry.AgentNameRegistry |
| 修改 | `src/micodeagent/tool/filter.py` | 新增 TEAMMATE_ALLOWED_TOOLS,扩展 FilterParams 加 teammate bool |
| 新建 | `src/micodeagent/command/builtin_team.py` | /team list/info/delete/kill 4 个命令 |
| 修改 | `src/micodeagent/tui/app.py` 与相关文件 | 接收 team_mgr、coordinator 标签 |
| 修改 | `src/micodeagent/config/__init__.py` | Features 字段新增 coordinator_mode 与 fork_teammate |
| 修改 | `src/micodeagent/cli/__init__.py` | wire team.Manager / coordinator,注册 7 个工具 |
| 新建 | `src/micodeagent/cli/team_member.py` | --team-member 自治循环 |
| 修改 | `.micodeagent/config.yaml.example` | 加示例 features 段(可选,不强制) |

## T1: 基础类型 — `src/micodeagent/team/types.py`

**文件:** `src/micodeagent/team/types.py`
**依赖:** 无
**步骤:**
...
**验证:** `python -c "from micodeagent.team.types import Team, TeammateInfo, BackendType"` 不报错;`ruff check src/micodeagent/team/types.py` 通过

## T2: sanitize 与原子写 — `src/micodeagent/team/persistence.py`

**文件:** `src/micodeagent/team/persistence.py`
**依赖:** T1
**步骤:**
...
## T3: Manager 与持久化 — `src/micodeagent/team/manager.py`

**文件:** `src/micodeagent/team/manager.py`
**依赖:** T1, T2
**步骤:**
1. 定义 `class Manager`(F3)
2. 实现 `Manager.__init__(home_dir, project_root, wt_mgr, task_mgr, reg)`(F4)
   - 创建 `<home_dir>/.micodeagent/teams/` 目录
...
## T3b: Team.Manager 跨进程并发兜底 — 继续 `src/micodeagent/team/manager.py` + `src/micodeagent/team/persistence.py`

**文件:** `src/micodeagent/team/manager.py`(为 add_member / set_member_active 加 reload-before-modify)、`src/micodeagent/team/persistence.py`(增加 `reload_from_disk_locked`)
**依赖:** T3
**步骤:**
...
## T4: Team 成员操作 — 继续 `src/micodeagent/team/manager.py`

**文件:** `src/micodeagent/team/manager.py`(同 T3)
**依赖:** T3
**步骤:**
...
## T5: mailbox 文件锁 — `src/micodeagent/team/mailbox/lock.py`

**文件:** `src/micodeagent/team/mailbox/lock.py`
**依赖:** 无
**步骤:**
...
## T6: mailbox Message 与 Box — `src/micodeagent/team/mailbox/__init__.py` + `message.py`

**文件:** `src/micodeagent/team/mailbox/__init__.py`、`src/micodeagent/team/mailbox/message.py`
**依赖:** T5
**步骤:**
1. `message.py` 定义 `MessageType(StrEnum)` 与 5 个常量(F32)
...
5. 实现 `async Box.write(agent_id, msg)`(F33)
   ...
   - 读 `<dir>/<agent_id>.json`(不存在视为 `[]`)
   - 追加 msg(若 timestamp 为空设为 `datetime.now(timezone.utc).isoformat()`)
   - `atomic_write_json`
...
## T7: AgentNameRegistry — `src/micodeagent/team/registry/__init__.py`

**文件:** `src/micodeagent/team/registry/__init__.py`
**依赖:** 无
**步骤:**
...
## T8: tasks Store — `src/micodeagent/team/tasks/__init__.py`

**文件:** `src/micodeagent/team/tasks/__init__.py`
**依赖:** T5(用 mailbox 的 lock)
**步骤:**
1. 定义 `Status(StrEnum)` / `Task` / `Filter` / `Patch` 类型(F30)
2. 定义 `class Store`,字段 `_path: str`, `_lock: asyncio.Lock`
3. 实现 `__init__(path)`
4. 实现 `async create(t) -> str`——生成 `task_<6 位 hex>` ID(`secrets.token_hex(3)`);read-modify-write `tasks.json`(用 lock 文件,路径 `<path>.lock`,复用 mailbox 的 `acquire_lock`——把它提到 `micodeagent.team.filelock` 共用包,或直接在 tasks 包内复制小段实现)
...
**注意:** 为减小循环依赖,把 `acquire_lock` 提到独立 `micodeagent.team.filelock` 模块,mailbox 与 tasks 共用。

**验证:** 单测覆盖 create/get/update;特别测 add_blocked_by 的双向更新

## T9: 共用 filelock 模块(从 mailbox 抽出)

**文件:** `src/micodeagent/team/filelock.py`(把 T5 实现迁过来)
**依赖:** 无
**步骤:**
1. 把 T5 的 `acquire_lock` 实现迁到 `micodeagent.team.filelock`,签名保持 `async def acquire(lock_path) -> AsyncContextManager[None]`
2. 在 `mailbox/lock.py` 改为 `from micodeagent.team.filelock import acquire`,删除本地实现
3. 在 `tasks/__init__.py` 也 import `filelock`

**验证:** `pytest tests/test_team_*.py` 全过

## T10: backend Protocol — `src/micodeagent/team/backend/__init__.py`

**文件:** `src/micodeagent/team/backend/__init__.py`
**依赖:** T1
**步骤:**
...
**验证:** `python -c "from micodeagent.team.backend import Backend, SpawnRequest, new_backend"` 通过

## T11: detect_backend — `src/micodeagent/team/backend/detect.py`

**文件:** `src/micodeagent/team/backend/detect.py`
**依赖:** T10
**步骤:**
...
## T12: tmux backend — `src/micodeagent/team/backend/tmux.py`

**文件:** `src/micodeagent/team/backend/tmux.py`
**依赖:** T10
**步骤:**
...
3. 实现 `async spawn(req)`(F15):
   ...
   - `cmd` 构造:`python -m micodeagent --team-member --team <team_name> --member <member_name> --agent-id <agent_id> --session-dir <session_dir> --worktree <wt_path> [--agent-type <type>] [--model <model>] [--plan-mode]`(可用 `shlex.quote` 转义)
...
**注意:** spawn 启动的 micodeagent CLI 需要支持 `--team-member` flag;这部分留给 T21(cli/__init__.py 改造)+ T29(team_member.py 新建)

**验证:** 单测断言命令字符串构造正确(用 monkeypatch.setattr 替换 `asyncio.create_subprocess_exec` 收 args);集成测试在 CI 跳过(需要 tmux)

## T13: iterm2 backend — `src/micodeagent/team/backend/iterm2.py`

**文件:** `src/micodeagent/team/backend/iterm2.py`
**依赖:** T10
**步骤:**
...
## T14: in-process backend — `src/micodeagent/team/backend/inprocess.py`

**文件:** `src/micodeagent/team/backend/inprocess.py`
...
## T15: feature flag — `src/micodeagent/team/feature.py`

**文件:** `src/micodeagent/team/feature.py`
**依赖:** 无
**步骤:**
...
## T16: TeammateContext — `src/micodeagent/agent/team_hook.py`

**文件:** `src/micodeagent/agent/team_hook.py`
**依赖:** 无
**步骤:**
...
**验证:** `python -c "from micodeagent.agent.team_hook import TeamHook, TeammateContext"` 通过

## T17: 队员专属工具白名单 — `src/micodeagent/tool/filter.py` 扩展

**文件:** `src/micodeagent/tool/filter.py`(修改)
**依赖:** 无
**步骤:**
1. 新增常量:
   ```python    TEAMMATE_EXTRA_TOOLS: list[str] = [        "TaskCreate", "TaskGet", "TaskList", "TaskUpdate", "SendMessage",    ]    ```
...
## T18: spawn_teammate 主流程 — `src/micodeagent/team/spawn.py`

**文件:** `src/micodeagent/team/spawn.py`
**依赖:** T1-T17
**步骤:**
...
1. 定义 `async Manager.spawn_teammate(req: TeamSpawnRequest) -> str`
   2. 实现 plan.md 中描述的步骤流程:
      ...
      - `await wt_mgr.create(f".micodeagent/worktrees/team-{sanitized}+{member}")`
      - 申请 session_dir(本期复用已有的 session 格式,自己生成新 id)
      ...
      - 若 backend_type=tmux/iterm2:`await Box(t.mailbox_dir).write(agent_id, Message(from_="lead", text=req.prompt))` 预写初始任务(F13)
...
2. 提供 helper `build_team_context_reminder(team, member, agent_id)` 构造 `<team-context>` reminder
3. 提供 helper `team_system_prompt_suffix() -> str` 返回 F39 附录

**验证:** 单测覆盖 in-process 后端的 spawn 全流程;Pane 后端的 spawn 用 mock backend

## T19: Agent 工具集成 — `src/micodeagent/agent/agent_tool.py` 修改

**文件:** `src/micodeagent/agent/agent_tool.py`(修改)
**依赖:** T16, T18
**步骤:**
...
**验证:** 单测:不带 team_name 走原有路径;带 team_name 调 mock team_hook,断言 spawn_teammate 被调

## T20: 队员 Loop incoming-messages 注入 — `src/micodeagent/agent/team_mailbox.py`

**文件:** `src/micodeagent/agent/team_mailbox.py`
**依赖:** T16, T6
**步骤:**
1. 在 `agent.Agent.run` / `run_to_completion` 的迭代头部(调 LLM 前),检查 ctx 中是否有 TeammateContext;实现位于 `micodeagent.agent.team_mailbox.ingest_team_mailbox`
...
## T21: task.Manager 改造 — `src/micodeagent/task/manager.py` 修改

**文件:** `src/micodeagent/task/manager.py`(修改)
**依赖:** T7
**步骤:**
1. `Manager` 持一个 `name_reg: AgentNameRegistry | None` 引用(可选 None 兜底)
2. `launch` 时:若 `name_reg` 非 None 且 name 非空,调 `name_reg.register(name, id_)`;同时保持本地 `_by_name` 兜底(避免破坏既有调用)
...
## T22: 协作工具实现 — `src/micodeagent/team/tools/`

**文件:** `team_create.py` / `team_delete.py` / `task_create.py` / `task_get.py` / `task_list.py` / `task_update.py` / `send_message.py`
**依赖:** T3, T6, T7, T8
**步骤:**
...
8. `SendMessage`(F34):参数 `to`/`message`/`type`/`request_id`/`approve`;`execute` 调 `await mailbox.write` + `await backend.wake` + 续派检测
...
**文件:** `tests/test_tool_filter.py`(修改)
**依赖:** T17, T22
**步骤:**
...
## T24: coordinator 包 — `src/micodeagent/coordinator/__init__.py`

**文件:** `src/micodeagent/coordinator/__init__.py`
**依赖:** 无
**步骤:**
...
## T25: config 加 features — `src/micodeagent/config/__init__.py` 修改

**文件:** `src/micodeagent/config/__init__.py`(修改)
**依赖:** 无
**步骤:**
...
## T26: TUI 集成 — `src/micodeagent/tui/app.py` 修改

**文件:** `src/micodeagent/tui/app.py` 与可能的 view 文件(修改)
**依赖:** T3, T24
**步骤:**
1. `MiCodeAgentApp.__init__` 加 `team_mgr: team.Manager`、`coordinator_mode: bool = False`
2. `MiCodeAgentApp` 加字段 `coordinator_mode: bool` 与 `lead_mail_event: asyncio.Event`;`__init__` 时 `lead_mail_event = asyncio.Event()`
3. coordinator 应用迁到 `src/micodeagent/cli/__init__.py` 中的 main_agent 上(`set_allowed_tools` + `append_system_prompt`)——tui 自身只负责状态栏渲染
4. 状态栏渲染时若 `app.coordinator_mode is True` 在 mode label 后追加 ` [COORDINATOR]`(参见 `src/micodeagent/tui/view.py status_bar()`)
...
## T27: /team slash 命令 — `src/micodeagent/command/builtin_team.py`

**文件:** `src/micodeagent/command/builtin_team.py`
**依赖:** T3
**步骤:**
...
## T28: cli wire — `src/micodeagent/cli/__init__.py` 修改

**文件:** `src/micodeagent/cli/__init__.py`(修改)
**依赖:** T1-T27
**步骤:**
...
6. 构造 `MiCodeAgentApp(..., team_mgr=team_mgr, coordinator_mode=coordinator.is_enabled(cfg))`
...
**验证:** `python -m micodeagent` 主流程能启动 TUI;`ruff check src/micodeagent/cli/` 通过

## T29: --team-member 自治循环 — `src/micodeagent/cli/team_member.py`(新文件)

**文件:** `src/micodeagent/cli/team_member.py`(新建)
**依赖:** T28
**步骤:**
...
**文件:** `src/micodeagent/cli/__init__.py`(修改)+ `src/micodeagent/team/manager.py`(加 helper)
**依赖:** T21, T3
**步骤:**
1. 在 `cli/__init__.py` wire 后,注册 `on_task_done` 回调到 `task_mgr`:
   ```python    async def _on_done(task_id: str) -> None:        await team_mgr.handle_task_done(task_id)    task_mgr.on_task_done(_on_done)    ```
2. 实现 `async Manager.handle_task_done(agent_id)`:
   ...
   - `await mailbox.write(lead_agent_id, Message(from_=name, text=f"[idle] {name} (reason: available)"))`

**验证:** 集成测试:in-process 后端 spawn 队员 → 自然结束 → 断言 Team.config 中 `is_active=False`、Lead mailbox 有 idle 消息

## T30b: Lead mailbox 轮询 + 自动唤醒 — `src/micodeagent/team/manager.py` + `src/micodeagent/tui/tasks.py` + `src/micodeagent/tui/app.py` + `src/micodeagent/tui/stream.py`

**文件:**
- `src/micodeagent/team/manager.py`(增加 `poll_lead_mailboxes` + `LeadMessage`)
- `src/micodeagent/tui/tasks.py`(增加 `consume_lead_mail` / `wait_for_lead_mail` / `build_team_update_reminder` / `lead_mail_message`)
- `src/micodeagent/tui/app.py`(`on_mount` 启动 watcher;`on_message` 处理 `LeadMailMessage`)
- `src/micodeagent/tui/stream.py`(增加 `begin_autonomous_turn`)
**依赖:** T28(cli 已 wire team_mgr 进 TUI 参数)
**步骤:**

1. `team.Manager.poll_lead_mailboxes()`:遍历 `m.list_()`,对每个 Team 用 `Box(t.mailbox_dir).read_unread(t.lead_agent_id)` 读未读,标 read,返回 `list[LeadMessage(team_name, from_, type, content, time)]`
...
## T31: 续写检测 — `src/micodeagent/team/tools/send_message.py`

**文件:** `src/micodeagent/team/tools/send_message.py`(同 T22)
**依赖:** T22, T21
**步骤:**
1. `SendMessage.execute` 写完邮箱后:
   ...
   - 若 backend_type=in-process:
     - 查 `task_mgr.get(agent_id)`,若 `status != Running`:
       ...
       - `await task_mgr.send_message(ctx, name, content)` 走已有的续派接口
   - 若 Pane 后端:已通过 wake 唤醒,无需续派

**验证:** 单测:先 spawn → 等结束 → SendMessage → 断言 task 重新 Running

## T32: Plan 审批权限切换 — `src/micodeagent/agent/team_mailbox.py`

**文件:** `src/micodeagent/agent/team_mailbox.py`(修改)
**依赖:** T20
**步骤:**
...
4. 可选:`mypy src/micodeagent/team/` 全绿
...
1. 启动 tmux:`tmux new-session -s worktree-test`
2. 在内层跑 `cd /path/to/micodeagent && uv run python -m micodeagent`(或 `micodeagent` 装好的入口)
3. 在 TUI 输入:「创建一个名为 demo 的团队」
4. 观察:
      - Agent 调 TeamCreate
      - `~/.micodeagent/teams/demo/config.json` 落地
      - 状态栏 / 输出确认成功
5. 在 TUI 输入:「派 alice 用 general-purpose,在 worktree 里 echo hello > /tmp/test_alice.txt」
   6. 观察:
      - tmux split 出新 pane
      - alice pane 内 micodeagent 子实例启动
      - `.micodeagent/worktrees/team-demo+alice/` 创建
