...
| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `src/micodeagent/llm/__init__.py` | 新增 `Usage` 类型；`StreamEvent` 加 `usage`；`Provider.stream` 加 `system_suffix` 形参 |
| 修改 | `src/micodeagent/llm/anthropic_provider.py` | `_effective_system(suffix)`；流结束上抛 usage |
| 修改 | `src/micodeagent/llm/openai_provider.py` | `stream_options={"include_usage": True}`；`_to_openai_messages` 拼 suffix；上抛 usage |
| 修改 | `src/micodeagent/tool/__init__.py` | `Tool` Protocol 加 `read_only` 属性 |
| 修改 | `src/micodeagent/tool/registry.py` | `read_only_definitions`、`is_read_only` |
| 修改 | `src/micodeagent/tool/{read_file,write_file,edit_file,bash,glob,grep}.py` | 各加 `read_only` 属性 |
| 修改 | `src/micodeagent/conversation.py` | `last_role()` |
| 修改 | `src/micodeagent/prompt.py` | `PLAN_MODE_REMINDER`、`EXECUTE_DIRECTIVE`；`SYSTEM_PROMPT` 增循环约定 |
| 重写 | `src/micodeagent/agent/__init__.py` | ReAct 循环、`Mode`、`execute_batched`、`usage/iter/notice` 事件、历史收尾 |
| 重写 | `tests/test_agent.py` | 多轮 fake provider、并发分批、停止条件、Plan 工具集 |
| 修改 | `tests/test_conversation.py` | `last_role` 断言 |
| 修改 | `src/micodeagent/tui/{app,stream,view}.py` | `mode`、per-turn cancel、Esc/Ctrl+C、`/plan` `/do`、`usage/iter/notice`/多工具、状态栏、动态区 |

## T1: llm 新增 Usage 类型（纯增量）**文件：** `src/micodeagent/llm/__init__.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.llm import Usage, StreamEvent; print(StreamEvent(usage=Usage(1,2)))"` 通过；`ruff check src/micodeagent/llm/__init__.py` 无告警（纯增字段，向后兼容，不改 Protocol 签名）。

## T2: tool 只读分类**文件：** `src/micodeagent/tool/__init__.py`、`src/micodeagent/tool/registry.py`、`src/micodeagent/tool/{read_file,write_file,edit_file,bash,glob,grep}.py`
**依赖：** 无
**步骤：**
...
**验证：** `pytest tests/test_tool.py`（若已存在）不回归；`python -c "from micodeagent.tool.registry import Registry"` 不报错；`ruff check src/micodeagent/tool` 无告警。

## T3: conversation.last_role**文件：** `src/micodeagent/conversation.py`、`tests/test_conversation.py`
**依赖：** 无
**步骤：**
...
## T4: prompt 计划态提示与循环约定**文件：** `src/micodeagent/prompt.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.prompt import PLAN_MODE_REMINDER, EXECUTE_DIRECTIVE; print(EXECUTE_DIRECTIVE)"` 通过；`pytest` 全量不回归。

## T5: llm Provider.stream 加 system_suffix + 用量上抛**文件：** `src/micodeagent/llm/__init__.py`、`src/micodeagent/llm/anthropic_provider.py`、`src/micodeagent/llm/openai_provider.py`、`src/micodeagent/agent/__init__.py`（临时补参）
**依赖：** T1
**步骤：**
...
4. `src/micodeagent/agent/__init__.py`：把现有 `stream_once` 里唯一的 `provider.stream(conv.messages(), defs)` 调用补成 `provider.stream(conv.messages(), defs, "")` 以匹配新签名——本步即让**非测试构建**保持绿（T6 会整体重写 agent）。
...
**验证：** `python -c "from micodeagent.llm.anthropic_provider import AnthropicProvider; from micodeagent.llm.openai_provider import OpenAIProvider"` 不报错；`ruff check src/micodeagent/llm` 无告警；`python -m micodeagent` 发一条纯文本回复正常（用量已随流上抛，旧 agent 暂未消费）。

## T6: agent ReAct 循环重写**文件：** `src/micodeagent/agent/__init__.py`、`tests/test_agent.py`
**依赖：** T1, T2, T3, T4, T5
**步骤：**
1. `agent/__init__.py`：
...
## T7: tui 接入 Agent Loop + 收尾 run 调用方**文件：** `src/micodeagent/tui/app.py`、`src/micodeagent/tui/stream.py`、`src/micodeagent/tui/view.py`
**依赖：** T4, T6
**说明：** T6 改了 `Agent.run` 签名（加 `mode` 与 `cancel`），其调用方 `tui/stream.py` 在此步同步更新——本步完成后 `python -m micodeagent` 才在**仓库级**重新可启动（T6 后只保证 agent 模块及其测试绿）。
**步骤：**
1. `app.py`：
   - `MiCodeAgentApp` 新增字段：`mode: agent.Mode = Mode.NORMAL`、`iter: int = 0`、`usage_in: int = 0`、`usage_out: int = 0`、`cur_tools: list[ToolDisplay] = []`（移除单个 `cur_tool`）、`turn_cancel: asyncio.Event | None = None`。
...
**验证：** `python -m micodeagent`（仓库级可启动）；`ruff format --check src/micodeagent/tui`、`ruff check src/micodeagent/tui` 无告警。

## T8: 全量验证与端到端冒烟**文件：** 无（验证）
**依赖：** T1–T7
**步骤：**
1. `ruff format --check .`、`ruff check .`、`pytest`、（可选）`mypy src/micodeagent`。
2. 端到端（openai 兼容端点，用 `.micodeagent/config.yaml`）：
