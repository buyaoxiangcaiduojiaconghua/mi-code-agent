...
- 新建包 `src/micodeagent/command/`：纯领域逻辑,不依赖 textual
- 既有包 `src/micodeagent/tui/`：删掉 `commands.py` 里的旧注册表与 5 个旧 handler;改为构造 Registry、实现 UI Protocol、把分发结果桥接回 App
- 自动补全菜单是 TUI 层独有的 UX 元素,完全在 `src/micodeagent/tui/complete.py` 中实现,只读 Registry 拿候选列表
...
handler 通过该协议操作 TUI;`MiCodeAgentApp` 实现此协议。
...
### `src/micodeagent/command/builtin_ui.py`
...
### `src/micodeagent/command/builtin_prompt.py`
...
### `src/micodeagent/command/builtins.py`
...
### `src/micodeagent/tui/commands.py` (改造)

**职责**：变成 thin glue:

1. 给 `MiCodeAgentApp` 实现 `command.UI` Protocol 的所有方法(每个方法 1~5 行,属性桥接 + `RichLog.write`)
2. 提供 `MiCodeAgentApp.dispatch_slash(text: str) -> bool`:调 `command.parse` → `self.cmd_registry.lookup` → 找到则 `await cmd.handler(self)`、未找到则向 RichLog 追加未知命令提示;返回 `True` 表示"已处理为命令"
3. 删掉 `builtin_commands` dict、`handle_exit/handle_plan/handle_do/handle_compact/handle_unknown` 等 5 个老 handler;保留 `handle_resume` 中和 `open_resume_menu` UI 方法整合的部分(ch09 写的 list/state 启动逻辑搬到 `MiCodeAgentApp.open_resume_menu`)
   **依赖**：`micodeagent.command`、`micodeagent.permission`、`micodeagent.prompt`。

### `src/micodeagent/agent/runtime.py` (改动)
...
### `src/micodeagent/tui/stream.py` (改动)

- `submit()` 协程:把 `dispatch_command(text)` 这一行替换为 `await self.dispatch_slash(text)`,其余流程不变(空输入早返回、非命令走 `conv.add_user + begin_turn`)

### `src/micodeagent/tui/complete.py` (新)

**职责**：自动补全菜单状态机 + 渲染。

- `CompletionMenu` dataclass 与方法见上面"核心数据结构"
- 提供 `MiCodeAgentApp._handle_completion_key(event: events.Key) -> bool`:当菜单激活时返回 `True` 表示该键已被菜单消费;否则返回 `False` 让上层透传给 TextArea
- 提供 `MiCodeAgentApp._sync_completion_from_input()`:每次 TextArea 内容变化后调用,根据当前内容刷新 `menu.active` / `menu.items`

### `src/micodeagent/tui/app.py` (改动)

- `MiCodeAgentApp` 增属性:`cmd_registry: Registry`、`completion: CompletionMenu`(注意:不要与已有的 `self.tool_registry: tool.Registry` 混淆,后者保持原名)
...
### `src/micodeagent/tui/view.py` (改动)
...
├── resume.py          改: 把 handle_resume 函数体迁到 (MiCodeAgentApp).open_resume_menu() 方法; 删除老 handle_resume
└── tests/test_tui.py  改: 旧的 test_tui_slash_compact_routes_to_command 等用例迁到新分发器

src/micodeagent/agent/runtime.py  改: 新增 SessionRuntime.reset_for_new_session(ses_ctx); clear_and_new_session 调用此 helper 重置 compact 子状态

src/micodeagent/__main__.py 不变 (TUI 构造时内部 wire cmd_registry)
src/micodeagent/prompt.py   READY_HINT 由"硬编码列表" 改为"建议输入 /help 查看可用命令" (去掉与命令清单同步的负担)
...
| 决策点                  | 选择                                                                                                                                                                                                                                                                                                            | 理由                                                                                                      |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 命令系统包归属          | 新建`src/micodeagent/command/`,不留在 `src/micodeagent/tui/`                                                                                                                                                                                                                                                            | tui 内 handler 持有 App 引用紧耦合,要满足 G3 必须把命令逻辑搬出 tui 包                                    |
...
| Handler 函数签名        | `async def handler(ui: UI) -> None` 而非 `def handler(app: MiCodeAgentApp) -> None`                                                                                                                                                                                                                                 | 满足 G3 解耦;handler 抛异常时,dispatch_slash 自动`RichLog.write(error_block(str(exc)))`,用户能看到失败    |
