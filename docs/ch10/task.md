...
> 包名：`micodeagent`（Python 3.12+）。源码位于 `src/micodeagent/`，内部模块以 `micodeagent.xxx` 导入；单测放在 `tests/`，运行框架 `pytest`。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/command/__init__.py` | 包出口,re-export 公共类型与函数 |
| 新建 | `src/micodeagent/command/command.py` | Kind 枚举、Command 与 Handler 类型 |
| 新建 | `src/micodeagent/command/registry.py` | Registry: register/lookup/visible/prefix_match + 冲突检测 |
| 新建 | `tests/test_command_registry.py` | 注册、冲突、前缀匹配、visible 排序的单测 |
| 新建 | `src/micodeagent/command/dispatch.py` | parse(input_text) — 解析 `/<name>` 形态 |
| 新建 | `tests/test_command_dispatch.py` | parse 各种输入的单测 |
| 新建 | `src/micodeagent/command/ui.py` | UI Protocol + NopUI 测试桩 |
| 新建 | `src/micodeagent/command/builtin_local.py` | 5 条纯本地命令(/help /status /memory /permission /session) |
| 新建 | `src/micodeagent/command/builtin_ui.py` | 5 条影响界面命令(/exit /plan /compact /resume /clear) |
| 新建 | `src/micodeagent/command/builtin_prompt.py` | 2 条提示词命令(/do /review) + REVIEW_DIRECTIVE 常量 |
| 新建 | `src/micodeagent/command/builtins.py` | register_builtins(reg) 把 12 条命令一次性注入 |
| 新建 | `tests/test_command_builtins.py` | 12 条命令注册成功、NopUI 调用全部 handler 不抛 |
| 改造 | `src/micodeagent/tui/commands.py` | 删旧 builtin_commands + handle_* 函数;新增 App 实现 UI Protocol 的全部方法 + dispatch_slash 入口 |
| 新建 | `src/micodeagent/tui/complete.py` | CompletionMenu 类型 + _handle_completion_key + _sync_completion_from_input + render |
| 改造 | `src/micodeagent/tui/resume.py` | handle_resume 改名/拆分为 MiCodeAgentApp.open_resume_menu(UI Protocol 实现) |
| 改造 | `src/micodeagent/tui/app.py` | App 增 cmd_registry+completion 属性; __init__ 构造 registry; on_key 在 IDLE 接入补全键位拦截 |
| 改造 | `src/micodeagent/tui/stream.py` | submit() 把 dispatch_command 调用替换为 self.dispatch_slash |
| 改造 | `src/micodeagent/tui/view.py` | 在 TextArea 下、状态栏上插入 Static#completion |
| 改造 | `tests/test_tui.py` | 老的 test_tui_slash_compact_routes_to_command / test_tui_unknown_slash_command_friendly 迁到新分发器 |
| 改造 | `src/micodeagent/prompt.py` | READY_HINT 改为"建议输入 /help 查看命令"(不再列具体命令名) |
| 改造 | `tests/test_prompt.py` | 跟随 READY_HINT 变化 |
| 改造 | `src/micodeagent/memory/manager.py` + `tests/test_memory_manager.py` | 新增 list_files |
| 改造 | `src/micodeagent/session/writer.py` + `tests/test_session_writer.py` | 新增 path 属性 |
| 改造 | `src/micodeagent/tool/registry.py` | 新增 count() |
| 改造 | `src/micodeagent/agent/runtime.py` + `tests/test_agent_runtime.py` | 新增 reset_for_new_session |

## 任务### T0a: memory.Manager.list_files**文件**：`src/micodeagent/memory/manager.py`、`tests/test_memory_manager.py`
**依赖**：无
**步骤**：
...
### T0b: session.Writer.path**文件**：`src/micodeagent/session/writer.py`、`tests/test_session_writer.py`
**依赖**：无
**步骤**：
...
### T0c: SessionRuntime.reset_for_new_session + tool.Registry.count**文件**：`src/micodeagent/agent/runtime.py`、`tests/test_agent_runtime.py`、`src/micodeagent/tool/registry.py`
**依赖**：无
**步骤**：
...
**验证**：`pytest tests/test_agent_runtime.py -k reset -v` 全绿;`python -c "from micodeagent.tool.registry import Registry; Registry().count()"` 不报错

### T1: 定义 Command 类型与 Kind 枚举**文件**：`src/micodeagent/command/command.py`
**依赖**：无
**步骤**：
1. 新建包 `micodeagent.command`(`__init__.py` 留空)
...
**验证**：`python -c "from micodeagent.command.command import Kind, Command"` 不报错

### T2: 实现 Registry + 冲突检测 + 前缀匹配**文件**：`src/micodeagent/command/registry.py`、`tests/test_command_registry.py`
**依赖**：T1
**步骤**：
...
### T3: parse 输入解析**文件**：`src/micodeagent/command/dispatch.py`、`tests/test_command_dispatch.py`
**依赖**：无
**步骤**：
...
### T4: UI Protocol + NopUI 测试桩**文件**：`src/micodeagent/command/ui.py`
**依赖**：无(声明 UI Protocol 让 T1 的 Handler 类型签名合法)
**步骤**：
1. import `from micodeagent.permission import Mode`
...
**验证**：`python -c "from micodeagent.command.ui import UI, NopUI; NopUI().mode()"` 不报错

### T5: 实现 5 条纯本地命令**文件**：`src/micodeagent/command/builtin_local.py`
**依赖**：T1、T2、T4
**步骤**：
...
2. `async def handle_status(ui)`:6 行 key:value,key 列宽固定(`Mode:`/`Tokens:`/`Tools:`/`Memories:`/`Model:`/`Directory:` 中最长那个);值依次:`ui.mode().value`、`f"{ui.usage_in()} in / {ui.usage_out()} out"`、`f"{ui.tool_count()} enabled"`、`f"{len(ui.memory_files())} files"`、`ui.model_name()`、`ui.cwd()`;首行加标题 "MiCodeAgent Status"(空行隔开)
...
**验证**：`python -c "import micodeagent.command.builtin_local"` 通过;后续 T8 的 test_command_builtins 会覆盖

### T6: 实现 5 条影响界面命令**文件**：`src/micodeagent/command/builtin_ui.py`
**依赖**：T1、T4
**步骤**：
...
**验证**：`python -c "import micodeagent.command.builtin_ui"` 通过

### T7: 实现 2 条提示词命令**文件**：`src/micodeagent/command/builtin_prompt.py`
**依赖**：T1、T4
**步骤**：
1. 模块级 `REVIEW_DIRECTIVE = "请审查当前上下文中的代码变更/已读取的文件,指出潜在 bug、可读性问题和可简化处。"`
2. `async def handle_do(ui)`: `ui.set_mode(Mode.DEFAULT); ui.inject_and_send("/do", prompt.EXECUTE_DIRECTIVE)` (import `from micodeagent import prompt`)
3. `async def handle_review(ui)`: `ui.inject_and_send("/review", REVIEW_DIRECTIVE)`

**验证**：`python -c "import micodeagent.command.builtin_prompt"` 通过

### T8: register_builtins + 12 条命令一次性注册**文件**：`src/micodeagent/command/builtins.py`、`tests/test_command_builtins.py`
**依赖**：T5、T6、T7
**步骤**：
...
**验证**：`pytest tests/test_command_builtins.py -v` 全绿;`ruff check src/micodeagent/command/` 无告警

### T8.5: App 属性铺垫**文件**：`src/micodeagent/tui/app.py`
**依赖**：T8
**步骤**：
1. `MiCodeAgentApp` 在原 `tool_registry: tool.Registry` 属性之后增 `cmd_registry: Registry | None = None`、`completion: CompletionMenu`、`_pending_println: list[str] = []`(注:Python 中 RichLog 可同步写,不像 Go 的 `tea.Cmd` 需要缓冲,但保留 list 以便 dispatch_slash 内统一收集 + 渲染时刷新)四个属性
2. 在 `__init__` 中初始化(`completion = CompletionMenu()`,`_pending_println = []`;registry 通过 T9c 注册)

**验证**：`python -m micodeagent` 在合法配置下能启动(空壳启动不被影响)

### T9a: TUI App 实现 UI 只读查询方法**文件**：`src/micodeagent/tui/commands.py`(在该文件已被清空旧 handler 后重写)
**依赖**：T8.5
**步骤**：
1. 删除旧文件 `src/micodeagent/tui/commands.py` 全部内容(`builtin_commands` dict、`dispatch_command` 函数、`handle_exit`/`handle_plan`/`handle_do`/`handle_compact`/`handle_unknown`、`format_compact_notice` 全部移除)
2. 给 `MiCodeAgentApp` 实现 UI Protocol 的所有只读方法(直接定义在 `app.py` 中也可,推荐放 `commands.py` 作为 mixin/方法集):
...
**验证**：`python -c "from micodeagent.tui.app import MiCodeAgentApp"` 通过

### T9b: TUI App 实现 UI 写入方法**文件**：`src/micodeagent/tui/commands.py`
**依赖**：T9a
**步骤**：
...
7. `def clear_and_new_session(self) -> None` — 步骤:
   ...
   b. `try: new_ses_ctx = compact.new_session_context(self._cwd)`;`except Exception as e: self.error(str(e)); return`。注意签名:`compact.new_session_context(workspace: str) -> SessionContext`,内部在 `<workspace>/.micodeagent/sessions/<id>/tool-results` 下建好目录
...
### T9c: dispatch_slash 入口 + 注册中心构造**文件**：`src/micodeagent/tui/commands.py`、`src/micodeagent/tui/app.py`
**依赖**：T9b
**步骤**：
...
**验证**：`python -m micodeagent`(合法配置)能启动且 `/help` 已可触发(肉测);`ruff check .` 无告警

### T10: open_resume_menu — handle_resume 重构进 UI Protocol**文件**：`src/micodeagent/tui/resume.py`
**依赖**：T9
**步骤**：
1. 把现有 `handle_resume(app: MiCodeAgentApp)` 函数体迁移到 `MiCodeAgentApp.open_resume_menu(self)`:把"state guard"那一段已经在 builtin_ui 的 handle_resume 处理;剩下"构造 session items 列表、设置 `self.resume_list`、切换 `self.state = SessionState.RESUMING`"放进 open_resume_menu;Textual 异步动作直接 `asyncio.create_task(...)` 即可。同时移除 open_resume_menu 内部对 `self.state != IDLE` 的判断和提示(guard 已在 dispatch_slash 按 Kind 统一处理)
...
**验证**：`python -c "from micodeagent.tui.resume import *"` 通过;肉测 `/resume` 仍弹历史列表

### T11: CompletionMenu 状态机 + 渲染**文件**：`src/micodeagent/tui/complete.py`(新)
**依赖**：T2
**步骤**：
...
8. `def _handle_completion_key(self: MiCodeAgentApp, event: events.Key) -> bool` (作为 App 方法,定义在 complete.py 但在 app.py 中绑定):if `not self.completion.active: return False`;match `event.key`:
...
9. `def _sync_completion_from_input(self: MiCodeAgentApp) -> None`:取 `self.input_area.text`,调 `self.completion.update(value, self.cmd_registry)`(注意是 `cmd_registry` 不是 `tool_registry`)

**验证**：`python -c "from micodeagent.tui.complete import CompletionMenu"` 通过;先用 `ruff check src/micodeagent/tui/` 看类型错误

### T12: TUI App 集成补全键位**文件**：`src/micodeagent/tui/app.py`
**依赖**：T9c、T10、T11
**步骤**：
...
**验证**：`python -m micodeagent` 启动后键入 `/` 应弹补全菜单(肉测);`ruff check .` 无告警

### T13: stream.submit 接入 dispatch_slash + view 渲染补全菜单**文件**：`src/micodeagent/tui/stream.py`、`src/micodeagent/tui/view.py`
**依赖**：T12
**步骤**：
...
**验证**：`python -m micodeagent` 整体跑通;`pytest tests/test_tui.py -k slash -v` 期待红(下一任务迁移测试)

### T14: 迁移测试 + READY_HINT 调整**文件**：`tests/test_tui.py`、`src/micodeagent/prompt.py`、`tests/test_prompt.py`
...
1. `cd /Users/codemelo/micodeagent && uv sync`(或 `pip install -e ".[dev]"`)
2. `tmux new-session -d -s mewspec 'uv run micodeagent'` 启动会话(或 `python -m micodeagent`)
