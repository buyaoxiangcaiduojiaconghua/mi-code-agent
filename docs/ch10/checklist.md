...
- [ ] /help、未命中提示、`prompt.READY_HINT` 三处均不出现硬编码命令清单(验证:`grep -rE "/(exit|plan|do|compact|resume|help|clear|review|status|memory|permission|session)" src/micodeagent/prompt.py src/micodeagent/tui/commands.py` 应只在新分发器代码内出现一次类型常量)
- [ ] 状态栏左侧 mode 徽章渲染与本 spec 实施前完全一致(验证:`git diff main -- src/micodeagent/tui/view.py | grep -E "(mode_label|mode_status_style|status_bar)"` 应无内部逻辑变更)
- [ ] /resume、/compact 沿用 ch09 的 `SessionState.IDLE` 限制(验证:T10 后跑既有 ch09 验收场景一遍)

## 编译与测试

- [ ] `python -m micodeagent` 在合法配置下能正常启动 TUI
- [ ] `pytest -q` 全部通过
- [ ] `ruff check .` 无告警
- [ ] `ruff format --check .` 通过(或本地 `ruff format .` 已统一格式)
- [ ] (可选)`mypy src/micodeagent` 通过

## 启动期冲突检测

- [ ] 在 `builtins.py` 中临时把某条命令注册两次,`python -m micodeagent` 立即抛 `RuntimeError` 退出,异常文本含具体冲突的命令名(验证:人工临时改动 + 跑 micodeagent + 还原)

## 端到端场景(tmux 实跑)

> 验证目标:在真实 tmux 会话内运行 micodeagent 进程,按 plan/spec 描述跑完整流程。
> 准备:`uv sync`(或 `pip install -e ".[dev]"`);开一个 `~/.micodeagent/memory/MEMORY.md` 保证 /memory 至少能列一条;`SESSIONS_DIR=$PWD/.micodeagent/sessions`(按 micodeagent 实际默认目录;如未自定义则使用 `${MEWCODE_HOME:-$PWD/.micodeagent}/sessions`)。

### 场景 A: 启动与 /help- [ ] **A1** 启动会话:`tmux new-session -d -s mewspec -x 200 -y 50 "uv run micodeagent 2>/tmp/micodeagent.err"`;`sleep 2`;`tmux capture-pane -t mewspec -p | grep -E "(MiCodeAgent|Send a message|DEFAULT|PLAN|default|plan)"` 至少命中 2 个关键字
...
### 场景 H: 启动期冲突检测- [ ] **H1** 临时编辑 `src/micodeagent/command/builtins.py`,把 /help 命令再注册一次同名;`python -m micodeagent 2>&1 | head -10`;输出含 "RuntimeError" 与 "help"(或具体冲突文本);还原 builtins.py

### 场景收尾- [ ] **/exit 端到端验证**:`tmux send-keys -t mewspec '/exit' Enter`;`sleep 1`;`tmux has-session -t mewspec 2>&1` 应报 "can't find session"
- [ ] **错误日志可观察**:`wc -l < /tmp/micodeagent.err` 输出为 0 或仅含白名单 info 行
