...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/prompt/modules.py` | `Module` 类型；`fixed_modules()` 七固定模块、`optional_modules()` 三空槽的内容常量 |
| 改   | `src/micodeagent/prompt/__init__.py` | `assemble_system`/`build_system_prompt`；删旧 `SYSTEM_PROMPT`/`PLAN_MODE_REMINDER`/`EXECUTE_DIRECTIVE` 常量（迁移）；保留 banner |
| 新建 | `src/micodeagent/prompt/environment.py` | `Environment` 结构、`gather_environment`、`Environment.render` |
| 新建 | `src/micodeagent/prompt/reminder.py` | `system_reminder` 标签包裹、`plan_reminder(full)`、规划提醒完整/精简常量、`EXECUTE_DIRECTIVE` |
| 新建 | `tests/test_prompt.py` | 装配顺序、跳空槽、N1 确定性、双重强化文本断言 |
| 改   | `src/micodeagent/tool/edit_file.py` | `DESCRIPTION` 补「编辑前先 `read_file`」 |
| 改   | `src/micodeagent/tool/bash.py` | `DESCRIPTION` 补「优先用专用工具而非 bash 拼凑」 |
| 改   | `src/micodeagent/llm/__init__.py` | `System` / `Request` dataclass；`Usage` 加缓存字段；`Provider.stream(req)`；删 `_effective_system` 与 prompt import |
| 改   | `src/micodeagent/llm/anthropic_provider.py` | 两块 system（稳定块打断点 + env 块）、缓存用量解析、reminder 并入末条 user |
| 改   | `src/micodeagent/llm/openai_provider.py` | 单条 system（stable+env 拼接）、`cached_tokens` 解析、reminder 追加尾部 user |
| 改   | `src/micodeagent/agent/agent.py` | `__init__(+version)`；`run` 采集环境/装配系统；按轮次 reminder；缓存用量透传 |
...
| 改   | `src/micodeagent/tui/stream.py` | `Agent(...)` 传 `self.version` |
| 改   | `examples/smoke.py` | 打印缓存用量；`Agent(p, registry, "dev")` |

---

## T1: prompt 模块化装配**文件：** `src/micodeagent/prompt/modules.py`、`src/micodeagent/prompt/__init__.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.prompt import build_system_prompt; print(build_system_prompt())"` 观察七模块按序、空槽不留空行；`ruff check src/micodeagent/prompt/` 无告警。

## T2: 环境采集与渲染**文件：** `src/micodeagent/prompt/environment.py`
**依赖：** 无
**步骤：**
...
## T3: 补充消息与规划提醒构造**文件：** `src/micodeagent/prompt/reminder.py`
**依赖：** 无
**步骤：**
...
## T5: 工具描述双重强化**文件：** `src/micodeagent/tool/edit_file.py`、`src/micodeagent/tool/bash.py`
**依赖：** 无
**步骤：**
...
**验证：** `pytest tests/test_tool.py` 仍通过；`ruff check src/micodeagent/tool/` 无告警。

## T6: llm 接口改造**文件：** `src/micodeagent/llm/__init__.py`
...
4. 删除 `_effective_system` 函数与对 `micodeagent.prompt` 的 import。
5. `new_provider` 工厂保持。

**验证：** `python -c "from micodeagent.llm import Request, System, Usage, Provider"` 不报错；anthropic/openai 适配器在 T7/T8 修复前会 import 失败，预期。

## T7: Anthropic 适配缓存通道 + reminder**文件：** `src/micodeagent/llm/anthropic_provider.py`
**依赖：** T6
**步骤：**
...
**验证：** `python -c "from micodeagent.llm.anthropic_provider import AnthropicProvider"` 不报错（配合 T8）；smoke anthropic 跑两轮见 `cache_read > 0`（次轮）。

## T8: OpenAI 适配缓存通道 + reminder**文件：** `src/micodeagent/llm/openai_provider.py`
**依赖：** T6
**步骤：**
...
## T9: agent 改造**文件：** `src/micodeagent/agent/agent.py`
**依赖：** T1, T2, T3, T6
**步骤：**
...
**验证：** `python -c "from micodeagent.agent.agent import Agent"` 不报错（配合 T10/T11）。

## T10: TUI 与 smoke 接线**文件：** `src/micodeagent/tui/stream.py`、`examples/smoke.py`

**依赖：** T9
**步骤：**

1. `stream.py` 中改 `Agent(self.provider, self.registry, self.version)`；`/do` 注入仍用 `prompt.EXECUTE_DIRECTIVE`（已迁至 `reminder.py`，但通过 `micodeagent.prompt` 包顶层重导出，import 路径不变）。
...
**验证：** `python -m micodeagent` 在合法配置下正常启动；`python examples/smoke.py` 能跑通。
...
3. `pytest`（全量单测通过）。
4. （可选）`mypy src/micodeagent` 通过子集检查。
5. `python -m micodeagent` 能正常启动。
