...
> 技术栈：Python 3.12+；使用 **官方 SDK** `mcp`（`pip install mcp` / `uv add mcp`，import 名 `mcp`）承载协议层（JSON-RPC 编解码、`initialize` 握手、stdio 与 Streamable HTTP 传输）。本章新增 **`micodeagent.mcp` 子包** 与入口装配，**不改 tool / agent / tui / permission / llm / config / conversation / prompt**。

## 架构概览

- **`micodeagent.mcp` 子包（新增）**：承载 MCP 客户端的全部职责——配置加载与两层合并、`${VAR}` 展开、字段校验、调用 SDK 建立 stdio / HTTP 会话、把远端工具适配成内置 `Tool` 协议、统一管理生命周期。仅依赖 `micodeagent.tool`、SDK 与标准库；不依赖 agent / tui / permission / conversation。
- **`micodeagent.cli`（改造）**：在 `tool.default_registry()` 之后、`permission.PermissionEngine(...)` 与 `MiCodeAgentApp(...).run()` 之前，加载 mcp 配置 → 启动 Manager → 把 Manager 产出的工具注册进 registry → 退出时 `await manager.close()`（包在 `try/finally` 中）。
- **`micodeagent.tool` 包（零改）**：`Registry.register` 与 `Tool` 协议本就是开放抽象，直接吃 `McpTool` 实例；`is_read_only` 对 MCP 工具返回正确值。
...
### `micodeagent.mcp.Config` / `micodeagent.mcp.ServerConfig`（对外）
```python
...
### 工具适配（包内私有）
```python
# McpTool 实现 micodeagent.tool.Tool 协议。
@dataclass
class McpTool:
...
# - root: 项目根（用来定位 <root>/.micodeagent.yaml）
...
# version 透传到 Implementation.version（便于 server 端识别 micodeagent 版本）。
...
- `async def _do_connect(mgr, name, srv, version)`：
  ...
  - 用一个**包级 `AsyncExitStack`**（挂在 `Manager._stack`）持有 transport 与 `ClientSession` 上下文：
    ...
    session = await mgr._stack.enter_async_context(
        ClientSession(read, write, client_info=Implementation(name="micodeagent", version=version))
    )
...
### `src/micodeagent/mcp/tool.py`
**职责：** 把 SDK 返回的 `mcp.types.Tool` 适配为 micodeagent `Tool` 协议。
**关键点：**
- 包级 `_VALID_NAME = re.compile(r"^[A-Za-z0-9_-]+$")`。
- 包级 `_non_text_warn_once: set[str] = set()`，记录已告警的 `full_name`。
- `def adapt_tool(server_name: str, t: mcp.types.Tool, session: CallerSession) -> McpTool | None`：
  ...
  - **禁用字符校验**：`_VALID_NAME.fullmatch(full_name)` 不通过 → 返回 `None` + stderr 告警。
  - `description`：`t.description` 为空时兜底 `f"来自 MCP server {server_name} 的工具 {t.name}"`。
  - `parameters`：`t.inputSchema` 转 `dict[str, Any]`；为空时给 `{"type": "object"}` 兜底。
  - `read_only`：`bool(t.annotations and t.annotations.readOnlyHint)`（None-safe）。
- `McpTool.name / description / parameters / read_only`：通过 dataclass 字段直接暴露。
- `async def McpTool.execute(self, args: dict[str, Any] | None) -> ToolResult`：
  - `arg_map = args if args else None`；
  - ```python
    try:
    ...
    ```
  - 遍历 `result.content`：`isinstance(block, mcp.types.TextContent)` → 收集 `block.text`；其余块计数，首次出现时 stderr 告警。
  - 用 `"\n".join(texts)` 拼出 `content`；返回 `ToolResult(content=content, is_error=bool(result.isError))`。

### `src/micodeagent/cli.py`（改造）
...
async def _amain() -> int:
    ...
    try:
        ...
        app = MiCodeAgentApp(cfg.providers, registry=registry, engine=engine)
        await app.run_async()
    finally:
        await mcp_mgr.close()
    return 0
...
| 配置文件位置 | 项目级 `<root>/.micodeagent.yaml` + 用户级 `~/.micodeagent/config.yaml` | 用户拍板；项目级 dotfile 一眼可见、与现有 `.micodeagent/config.yaml`（providers 凭据）分离 |
...
| 未定义变量 | 空串 + 一次性告警（不阻断） | server 自决无凭据时是否能跑；micodeagent 不替它拍板 |
...
cli._amain()
  ...
  ├─ await MiCodeAgentApp(...).run_async()
...
依赖方向（无环）：`micodeagent.cli → micodeagent.mcp → {micodeagent.tool, mcp(SDK), 标准库}`；`micodeagent.mcp` 不依赖 agent / tui / permission / conversation。
