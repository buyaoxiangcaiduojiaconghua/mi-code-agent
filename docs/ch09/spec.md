...
micodeagent 当前是无状态的：每次启动都是全新会话，不知道用户是谁、项目有什么规范、上次聊到哪里。ch08 解决了"单进程内长时间工作不崩"的问题，但进程一退出，所有对话历史和工作上下文就全丢了。
...
## 功能需求### 第 1 层：项目指令文件（MEWCODE.md）- **F1**：系统启动时按以下顺序扫描三个路径，找到就加载、找不到就跳过：① `<project_root>/MEWCODE.md`（项目级，最高优先级）；② `<project_root>/.micodeagent/MEWCODE.md`（项目配置级）；③ `~/.micodeagent/MEWCODE.md`（用户级，最低优先级）。三份文件的内容按此优先级顺序拼接——高优先级在前，模型优先遵循；各层之间用空行分隔。
...
- **F5**：`@include` 路径逃逸检测：解析后的绝对路径必须仍在当前指令文件所属的"根边界"之内——项目级文件（路径 ① ②）的根边界是 `<project_root>`；用户级文件（路径 ③）的根边界是 `~/.micodeagent/`。跳出边界的路径不加载，追加警告注释 `<!-- @include 路径超出允许范围，已跳过: <path> -->`。
...
- **F10**：`SessionContext` 新增 `session_dir` 字段（`<workspace>/.micodeagent/sessions/<session_id>`），原有 `spill_dir` 改为 `session_dir + "/tool-results"`。JSONL 存档路径为 `session_dir + "/conversation.jsonl"`。
...
- **F18**：`/resume` 触发后，扫描 `.micodeagent/sessions/` 下所有子目录，找到包含 `conversation.jsonl` 的有效会话。按最后修改时间倒序排列（最新在前）。
...
#### 会话清理- **F25**：进程启动时扫描 `.micodeagent/sessions/`，删除 session ID 中的时间戳距当前超过 30 天的会话目录（整个子目录，含 JSONL 和 tool-results）。
...
- **F29**：笔记分两级存放——项目级 `.micodeagent/memory/`，用户级 `~/.micodeagent/memory/`。项目级笔记记录与当前项目相关的信息（项目知识、参考资料），用户级笔记记录跨项目通用的信息（用户偏好、纠正反馈）。具体分级由 LLM 判断。
...
### 会话存档- **AC7（Session ID 格式）**：启动进程 → session ID 形如 `20260601-143022-a1b2`，`.micodeagent/sessions/` 下能找到对应目录。
...
### 自动笔记- **AC21（笔记创建）**：在对话中明确表达一个偏好（如"回复简洁点"）→ Agent 回复后，`.micodeagent/memory/` 或 `~/.micodeagent/memory/` 下出现对应类型的 .md 文件，frontmatter 包含 type、title、created。
