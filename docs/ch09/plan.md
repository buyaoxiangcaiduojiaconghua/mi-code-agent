...
| 新子包 | 职责 |
|------|------|
| `micodeagent.instructions` | 三层 MEWCODE.md 加载 + @include 展开 |
| `micodeagent.session` | JSONL 会话写入、列表扫描、加载恢复、过期清理 |
| `micodeagent.memory` | 笔记 CRUD、索引管理、异步 LLM 更新 |

| 已有模块 | 改动 |
|--------|------|
| `micodeagent.prompt` | `build_system_prompt` 接受 instructions/memory 参数 |
| `micodeagent.conversation` | 新增 on_append/on_replace 回调 |
| `micodeagent.compact.state` | session ID 格式改为 YYYYMMDD-HHMMSS-xxxx；`SessionContext` 加 `session_dir` 字段 |
| `micodeagent.agent` | 每 5 轮 run 结束后触发记忆更新 |
| `micodeagent.tui` | 新增 /resume 命令和 `RESUMING` 状态 |
| `micodeagent.cli` | 启动流程串联指令加载、记忆初始化、会话清理 |
...
class SessionContext:
    ...
    session_dir: str       # <workspace>/.micodeagent/sessions/<session_id>
...
cli.main()
  ...
  └─ MiCodeAgentApp(providers, ..., writer, mem_mgr, instruction_text, memory_text).run()
       └─ 选定 provider 后：
