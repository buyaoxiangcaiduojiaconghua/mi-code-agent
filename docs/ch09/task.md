...
> 包名：`micodeagent`（Python 3.12+）。源码位于 `src/micodeagent/`。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `src/micodeagent/compact/state.py` | session ID 格式变更、`SessionContext` 加 `session_dir`、`open_session_context` |
| 修改 | `src/micodeagent/conversation.py` | `from_messages` 类方法、回调触发 |
| 修改 | `tests/test_conversation.py` | 回调测试 |
| 修改 | `src/micodeagent/prompt.py` | `build_system_prompt` 签名变更 |
| 修改 | `src/micodeagent/prompt_modules.py` | `optional_modules` 改为接受参数 |
| 修改 | `tests/test_prompt.py` | 新签名测试 |
| 新建 | `src/micodeagent/instructions/__init__.py` | 子包标识 |
| 新建 | `src/micodeagent/instructions/loader.py` | Loader 类、三层加载、@include 展开 |
| 新建 | `tests/test_instructions_loader.py` | @include 深度/环路/逃逸/缺失文件测试 |
| 新建 | `src/micodeagent/session/__init__.py` | 子包标识 |
| 新建 | `src/micodeagent/session/writer.py` | Writer、Entry、append、`open_existing` |
| 新建 | `src/micodeagent/session/list.py` | `list_sessions`、SessionInfo |
| 新建 | `src/micodeagent/session/load.py` | `load_session`、坏行跳过、孤立截断 |
| 新建 | `src/micodeagent/session/cleanup.py` | `clean_expired`、ID 时间戳解析 |
| 新建 | `tests/test_session.py` | JSONL 读写、列表、恢复、清理测试 |
| 新建 | `src/micodeagent/memory/__init__.py` | 子包标识 |
| 新建 | `src/micodeagent/memory/types.py` | NoteType、Note、UpdateAction |
| 新建 | `src/micodeagent/memory/store.py` | Store、笔记文件 CRUD、索引读写 |
| 新建 | `src/micodeagent/memory/manager.py` | Manager、`load_index`、`update_async` |
| 新建 | `src/micodeagent/memory/prompts.py` | 记忆更新 prompt 模板 |
| 新建 | `tests/test_memory.py` | 索引加载、操作执行、截断测试 |
| 修改 | `src/micodeagent/agent/agent.py` | run 末尾触发记忆更新 |
| 修改 | `src/micodeagent/agent/runtime.py` | 接受 `memory_manager` 注入 |
| 修改 | `src/micodeagent/tui/commands.py` | /resume 命令注册 |
| 新建 | `src/micodeagent/tui/resume.py` | `RESUMING` 状态、会话列表项、`handle_resume` |
| 修改 | `src/micodeagent/tui/app.py` | `RESUMING` 状态集成、App 新增字段 |
| 修改 | `src/micodeagent/cli.py` | 启动流程串联 |
| 修改 | `.micodeagent/config.yaml.example` | 配置示例补充说明 |

## T1: Session ID 格式变更**文件：** `src/micodeagent/compact/state.py`
**依赖：** 无
**步骤：**
...
2. `SessionContext` 新增 `session_dir: str` 字段，值为 `<workspace>/.micodeagent/sessions/<session_id>`
...
## T2: Conversation 回调机制**文件：** `src/micodeagent/conversation.py`、`tests/test_conversation.py`
**依赖：** 无
**步骤：**
...
## T3: 项目指令加载器**文件：** `src/micodeagent/instructions/__init__.py`、`src/micodeagent/instructions/loader.py`、`tests/test_instructions_loader.py`
**依赖：** 无
**步骤：**
...
## T4: Session Writer**文件：** `src/micodeagent/session/__init__.py`、`src/micodeagent/session/writer.py`
...
**验证：** `python -c "from micodeagent.session.writer import Writer"` 不报错

## T5: 会话列表扫描**文件：** `src/micodeagent/session/list.py`
...
**验证：** `python -c "from micodeagent.session.list import list_sessions"` 不报错

## T6: 会话加载恢复**文件：** `src/micodeagent/session/load.py`
**依赖：** T4（Entry 类型）
**步骤：**
...
**验证：** `python -c "from micodeagent.session.load import load_session"` 不报错

## T7: 会话过期清理**文件：** `src/micodeagent/session/cleanup.py`
**依赖：** T1（`parse_session_time`）
**步骤：**
...
**验证：** `python -c "from micodeagent.session.cleanup import clean_expired"` 不报错
...
## T9: 笔记类型与存储**文件：** `src/micodeagent/memory/__init__.py`、`src/micodeagent/memory/types.py`、`src/micodeagent/memory/store.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.memory.store import Store"` 不报错

## T10: 记忆管理器**文件：** `src/micodeagent/memory/manager.py`、`src/micodeagent/memory/prompts.py`
**依赖：** T9
**步骤：**
...
**验证：** `python -c "from micodeagent.memory.manager import Manager"` 不报错
...
## T12: build_system_prompt 参数化**文件：** `src/micodeagent/prompt.py`、`src/micodeagent/prompt_modules.py`、`tests/test_prompt.py`
**依赖：** 无
**步骤：**
...
## T13: /resume 命令注册**文件：** `src/micodeagent/tui/commands.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.tui.commands import BUILTIN_COMMANDS"` 不报错

## T14: 会话列表 UI**文件：** `src/micodeagent/tui/resume.py`、`src/micodeagent/tui/app.py`
**依赖：** T5（`list_sessions`）, T13
**步骤：**
1. `app.py`：
   - `SessionState` 枚举新增 `RESUMING = "resuming"`
   - `MiCodeAgentApp` 新增字段：`writer: session.Writer`、`mem_mgr: memory.Manager`、`instruction_text: str`、`memory_text: str`、`sessions_dir: str`
...
**验证：** `python -c "from micodeagent.tui.resume import begin_resume"` 不报错

## T15: Agent 记忆更新触发**文件：** `src/micodeagent/agent/agent.py`、`src/micodeagent/agent/runtime.py`
**依赖：** T10（Manager）
**步骤：**
...
## T16: cli.py 启动流程串联**文件：** `src/micodeagent/cli.py`
...
5. 修改 `MiCodeAgentApp(...)` 调用：传入 `writer`、`mem_mgr`、`instruction_text`、`memory_text`
6. 在 TUI 的 provider 选定回调中：调 `mem_mgr.set_provider(provider, model)`

**验证：** `python -m micodeagent` 能启动；`ruff check src/micodeagent` 无告警；`mypy src/micodeagent` 无报错（可选）

## T17: 配置示例更新**文件：** `.micodeagent/config.yaml.example`
**依赖：** 无
**步骤：**
