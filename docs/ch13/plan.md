...
- 工具调用与权限引擎沿用 ch04~ch12 既有的 `micodeagent.agent` / `micodeagent.permission` / `micodeagent.hook` 模块
...
- `micodeagent.subagent.builtin/*.md` — 内置 3 个角色文件,`importlib.resources` 读取
...
from micodeagent.permission import PermissionMode
...
`src/micodeagent/agent/agent_tool.py`:
...
`src/micodeagent/tool/filter.py`:
...
- `MiCodeAgentApp.__init__` 加 `task_mgr: task.Manager`、`subagent_catalog: subagent.Catalog`(由 cli 注入)
...
`src/micodeagent/tui/skill_fork.py` 改造：

- 删除现有 `run_sub_agent` 内的零散逻辑
- 改为调 `subagent.launch_fork(host, opts, conv)`,host 持有 `self.task_mgr` / `self.runtime` / `self.engine` 等

## 模块设计### 模块 A:`micodeagent.subagent`**职责:**

- 数据结构 `Definition`
- Markdown + YAML 解析(复用 `skills/parser.py` 的 `parse_frontmatter_and_body`——抽到 `micodeagent.util.markdown` 让两方共用,或 skills 与 subagent 都各自有一份)
- 三层 + 内置随包加载
...
- `micodeagent.permission`(解析 permission_mode 字段)
- `pyyaml`
...
- 内置文件 `micodeagent/subagent/builtin/general-purpose.md` / `explore.md` / `plan.md` 通过 `importlib.resources.files("micodeagent.subagent.builtin")` 读取
- 加载错误统一 stderr `print(f"subagent {name}: ... skipped", file=sys.stderr)`

### 模块 B:`micodeagent.task`**职责:**
...
- `micodeagent.agent`(Agent)
- `micodeagent.conversation`
- `micodeagent.tool`
- `micodeagent.llm`
...
cli.main()
  ...
  ├── MiCodeAgentApp(..., task_mgr=task_mgr, subagent_catalog=subagent_catalog, ...)
  │     │
  │     └── 在 MiCodeAgentApp 内:Agent 工具的注册被推迟到主 Agent 构造后
  │         (因为要把 parent_agent 注入),或者 Agent 工具 lazy 拿:把 catalog / task_mgr 写死,
  │         parent_agent 通过函数 / 持有 self.app 拿
```

**简化方案:** Agent 工具在 `cli.main` 注册,parent 字段在 `MiCodeAgentApp` 构造完后回填:
...
task_mgr.launch 协程:
    ...
    try:
        ...
        ...
    ↓
MiCodeAgentApp._consume_task_done 协程:
    while True:
...
```
MiCodeAgentApp.execute("/foo") → skills.Executor.execute → fork 闭包 self._run_sub_agent
    ↓ (改造后)
...
| Agent 工具的 parent 注入时机        | `cli.main` 注册时为 None,`MiCodeAgentApp` 构造后 `set_parent` 回填                   | `Registry` 在 `MiCodeAgentApp` 之前已构造,Agent 工具的 parent 依赖 `app.main_agent` 反推         |
...
| 内置 .md 加载                       | `importlib.resources.files("micodeagent.subagent.builtin")`                          | 标准库官方推荐方式;打包成 wheel 后仍能读取;无需 manifest 配置                                |
