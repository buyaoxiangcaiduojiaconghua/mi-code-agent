...
- [ ] `micodeagent.subagent` 包存在且可导入(验证:`python -c "import micodeagent.subagent"`)
...
- [ ] `MiCodeAgentApp` 持有 `task_mgr` 与 `subagent_catalog` 字段(验证:`python -c "import micodeagent.tui.app"`)
...
- [ ] `cli.main` 注册 4 个 task 工具 + 1 个 Agent 工具,`subagent_catalog` 与 `task_mgr` 传给 `MiCodeAgentApp`(验证:`python -m micodeagent --help` 不报错)
...
- [ ] 项目可启动:`python -m micodeagent --help`
- [ ] lint 通过:`ruff check src/micodeagent/`
- [ ] 格式化通过:`ruff format --check src/micodeagent/`
- [ ] 类型检查通过(可选):`mypy src/micodeagent/`
...
### 场景 1:定义式子 Agent(Explore)前台同步**预置:** 无须额外配置。当前目录 `cd /Users/codemelo/micodeagent`。

**步骤:**
- [ ] tmux 启动 micodeagent:`tmux new-session -d -s ch13 -x 200 -y 50 "uv run micodeagent"`
- [ ] 给 LLM 输入:「用 Explore 子 Agent 找出 `src/micodeagent/permission` 包下所有以 `test_` 开头的函数,只统计数量,不要修改任何文件」
...
**步骤:**
- [ ] tmux 启动 micodeagent
- [ ] 第一轮:让 LLM 读一些文件铺垫上下文,如「读 `src/micodeagent/agent/agent.py` 头 50 行」
...
### 场景 3:主 Agent 用 TaskList / TaskGet 查询**预置:** 接场景 2 之后,或者重启 micodeagent 后先 launch 一个长跑任务。

**步骤:**
- [ ] 调用一个会跑较久的子 Agent:「用 `run_in_background=True`,让一个 general-purpose 子 Agent 阅读 `src/micodeagent` 下所有 `.py` 文件 head 200 行,生成总结」
...
.micodeagent/agents/auto-bash.md
----------------------------
...
**步骤:**
- [ ] tmux 启动 micodeagent(权限模式 default)
...
.micodeagent/agents/ask-bash.md
---------------------------
...
**步骤:**
- [ ] tmux 启动 micodeagent(权限模式 default,未预先批准 echo)
...
**步骤:**
- [ ] tmux 启动 micodeagent
- [ ] 输入:「用 `run_in_background=True name=worker1` 起一个 general-purpose 子 Agent,任务是『列出 `src/micodeagent/cli.py` 的 import 块』」
- [ ] 主 Agent 收到 task_id,等几秒后 task-notification 显示 Result(imports 列表)
- [ ] 输入:「调 SendMessage 给 worker1 发『再列出 `src/micodeagent/agent/agent.py` 头 20 行』」
...
**预置:** 创建 `.micodeagent/agents/wc-counter.md`:
```yaml
---
name: wc-counter
...
- [ ]  tmux 启动 micodeagent
- [ ]  输入:「用 Agent 工具调 `subagent_type=wc-counter`,任务: 统计 `README.md` 和 `src/micodeagent/cli.py` 的行数」
...
### 场景 11.6:自定义 Agent 字段错误降级**预置:** 创建 `.micodeagent/agents/bad.md` 含非法字段:
...
- [ ]  micodeagent 正常启动,不阻断
- [ ]  输入:「用 Agent 工具调 `subagent_type=bad`,任务: 回个 hi」
- [ ]  子 Agent 仍能正常跑(model 降级 inherit、mode 降级 default 后,工具集与权限按降级值)
- [ ]  **测试完删除 `.micodeagent/agents/bad.md`**### 场景 11:角色文件覆盖**预置:** 创建 `.micodeagent/agents/explore.md`:
...
- [ ]  删除 `.micodeagent/agents/explore.md`,重启 micodeagent,再次跑 → 不再含此标记
