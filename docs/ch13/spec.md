...
本章把上述能力补齐,让 micodeagent 从单 Agent 进化到可分发任务的主从架构。
...
- **F5**:Catalog 三层加载(本期插件级恒为空),顺序:
  1. 项目级:`<root>/.micodeagent/agents/*.md`
  2. 用户级:`~/.micodeagent/agents/*.md`
  3. 内置级:随包发布的 `micodeagent/subagent/builtin/*.md`(通过 `importlib.resources` 读取)
...
- **F10**:新增 Agent 构造选项(通过 `Agent.__init__` 关键字参数 / `dataclass` 字段):
  - `system_prompt: str`:子 Agent 启动时把 text 作为 system prompt 注入(覆盖默认 micodeagent 主 Agent 系统提示)
...
## 非功能需求- **N1**:工具列表稳定——主 Agent 看到的工具集不因 `.micodeagent/agents/` 增减或 Agent 工具被调用而变化(防止 prompt cache 抖动)
...
- **AC16**:项目级 `.micodeagent/agents/explore.md` 覆盖内置 `explore`,`resolve("explore")` 返回项目级版本
...
- **AC21**:全新项目级自定义 Agent(`.micodeagent/agents/<name>.md`)被 Catalog 加载;`subagent_type=<name>` 调用时,frontmatter 的 disallowedTools / permissionMode / maxTurns / SystemPrompt 全部生效——子 Agent 看不到黑名单工具、按指定 mode 决策、不超 turns、按 SystemPrompt 行事
- **AC22**:Agent 定义 frontmatter 的非法字段(unknown model / unknown permissionMode)在加载时 stderr 警告并 fallback 到默认值(model→inherit, mode→default),micodeagent 不阻断启动,该 Agent 仍可被 resolve 与调用
