...
> 包名：`micodeagent`(Python 3.12+)。源码位于 `src/micodeagent/`,内部模块以 `micodeagent.xxx` 导入。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/subagent/__init__.py` | 包公共导出 |
| 新建 | `src/micodeagent/subagent/definition.py` | `Definition` / `Source` 类型 |
| 新建 | `src/micodeagent/subagent/parser.py` | `parse_frontmatter_and_body` + `validate_meta` |
| 新建 | `tests/subagent/test_parser.py` | 解析与字段校验单测 |
| 新建 | `src/micodeagent/subagent/catalog.py` | `Catalog` + `load_catalog` / `resolve` / `list` / `fork_definition` |
| 新建 | `tests/subagent/test_catalog.py` | 多来源加载与覆盖测试 |
| 新建 | `src/micodeagent/subagent/embed.py` | `importlib.resources` 读取 builtin/*.md + `builtin_definitions()` |
| 新建 | `src/micodeagent/subagent/builtin/general-purpose.md` | 内置 general-purpose 定义 |
| 新建 | `src/micodeagent/subagent/builtin/explore.md` | 内置 Explore 定义 |
| 新建 | `src/micodeagent/subagent/builtin/plan.md` | 内置 Plan 定义 |
| 新建 | `tests/subagent/test_launch.py` | `launch_fork` 流程测试 |
| 新建 | `src/micodeagent/task/__init__.py` | 包公共导出 |
| 新建 | `src/micodeagent/task/manager.py` | `Manager` + `BackgroundTask` + `launch` / `adopt_running` / `stop` / `send_message` / `subscribe_done` |
| 新建 | `tests/task/test_manager.py` | 后台任务全生命周期测试 |
| 新建 | `src/micodeagent/task/tools.py` | 4 个内置工具 `TaskListTool` / `TaskGetTool` / `TaskStopTool` / `SendMessageTool` |
| 新建 | `tests/task/test_tools.py` | 4 个工具的单测 |
| 新建 | `src/micodeagent/agent/run_to_completion.py` | `run_to_completion` 方法实现(挂到 `Agent` 上) |
| 新建 | `tests/agent/test_run_to_completion.py` | `run_to_completion` / `dont_ask` / `max_turns` 测试 |
| 新建 | `src/micodeagent/agent/fork.py` | `build_forked_messages` + `is_fork_context` + `FORK_BOILERPLATE` |
| 新建 | `tests/agent/test_fork.py` | Fork 消息构造与上下文识别测试 |
| 新建 | `src/micodeagent/agent/agent_tool.py` | `AgentTool` + `execute` |
| 新建 | `tests/agent/test_agent_tool.py` | Agent 工具调用、嵌套阻断、超时切后台测试 |
| 新建 | `src/micodeagent/agent/permission_upgrade.py` | `ApprovalUpgrader` 类型 + `default_upgrader` |
| 新建 | `src/micodeagent/agent/launch.py` | `launch_fork` 公共启动函数(供 skill_fork 调用) |
| 新建 | `src/micodeagent/tool/filter.py` | `ALL_AGENT_DISALLOWED` / `ASYNC_AGENT_ALLOWED` / `apply_agent_tool_filter` |
| 新建 | `tests/tool/test_filter.py` | 过滤多层防线测试 |
| 新建 | `src/micodeagent/tui/tasks.py` | `_consume_task_done` + `build_task_notification` + ESC 切后台辅助 |
| 修改 | `src/micodeagent/agent/agent.py` | 加 system_prompt / max_turns / permission_mode / dont_ask / approval_upgrader 字段;`run` 抽 `_run_iter`;`_run_guarded` 加 `dont_ask` 短路 + `approval_upgrader` 升级 |
| 修改 | `tests/agent/test_agent.py` | 不破坏既有测试 |
| 修改 | `src/micodeagent/tool/registry.py` | 不动(过滤逻辑在 `filter.py`) |
| 修改 | `src/micodeagent/tui/app.py` | `MiCodeAgentApp` 加 `task_mgr` / `subagent_catalog`;`on_mount` 起 `_consume_task_done`;Agent 工具注册后 `set_parent` |
| 修改 | `src/micodeagent/tui/stream.py` | `_consume_stream` 加 ESC → `adopt_running` 分支 |
| 修改 | `src/micodeagent/tui/skill_fork.py` | 改造为调 `agent.launch_fork` |
| 修改 | `tests/tui/test_tui.py` | 补 ESC 切后台、task-notification 注入测试 |
| 修改 | `src/micodeagent/config.py` | 加 `enable_subagent_background: bool | None`(默认视为 True) |
| 修改 | `src/micodeagent/cli.py` | `load_catalog` / `Manager` / 4 个 task 工具注册 / Agent 工具注册 + `set_parent`;`task_mgr` / `subagent_catalog` 传给 `MiCodeAgentApp` |

## T1: subagent 包的 Definition 与 Source 类型**文件:** `src/micodeagent/subagent/definition.py`
**依赖:** 无
**步骤:**
1. 新建包 `micodeagent.subagent`,加 `definition.py`,声明 `Source(IntEnum)` 与四个常量:
   - `BUILTIN = 0`
   - `USER = 1`
   - `PROJECT = 2`
   - `PLUGIN = 3`(占位)
...
**验证:** `python -c "from micodeagent.subagent.definition import Definition, Source"` 导入无误

## T2: subagent 解析器**文件:** `src/micodeagent/subagent/parser.py`
**依赖:** T1
**步骤:**
1. 新建 `parser.py`,从 `skills/parser.py` 复制 `parse_frontmatter_and_body` 与 `UTF8_BOM` 常量(几乎不变,改为 `micodeagent.subagent` 包内调用)
...
## T4: 内置 Agent 定义文件**文件:** `src/micodeagent/subagent/builtin/{general-purpose,explore,plan}.md`
**依赖:** 无
**步骤:**
1. 创建目录 `src/micodeagent/subagent/builtin/`
2. `general-purpose.md`:
   ...
   你是 MiCodeAgent 的通用 Agent。根据用户的消息,使用可用工具完成任务。
...
## T5: subagent embed 与内置加载**文件:** `src/micodeagent/subagent/embed.py`
...
## T6: Catalog 与三层加载**文件:** `src/micodeagent/subagent/catalog.py`
...
2. 实现 `load_catalog(root: str) -> Catalog`:
   ...
   c._add_all(_load_from_dir(Path(root) / ".micodeagent/agents", Source.PROJECT), Source.PROJECT)
   return c
   ```
...
## T8: 工具过滤多层防线**文件:** `src/micodeagent/tool/filter.py`
...
**验证:** `python -c "from micodeagent.tool.filter import apply_agent_tool_filter"` 导入无误
...
## T10: Agent 类扩展 - 新增构造参数**文件:** `src/micodeagent/agent/agent.py`
...
**验证:** `python -c "from micodeagent.agent.agent import Agent; Agent.__init__"` 无 TypeError

## T11: ApprovalUpgrader 类型**文件:** `src/micodeagent/agent/permission_upgrade.py`
...
1. 新建文件,声明:
   ...
   from micodeagent.permission import PermissionOutcome
   from micodeagent.agent.approval import ApprovalRequest
...
**验证:** `python -c "from micodeagent.agent.permission_upgrade import ApprovalUpgrader"` 导入无误

## T12: Fork 路径辅助函数**文件:** `src/micodeagent/agent/fork.py`
...
## T14: `_run_guarded` 加 dont_ask 短路与 approval_upgrader**文件:** `src/micodeagent/agent/agent.py`
...
## T15: run_to_completion 实现**文件:** `src/micodeagent/agent/run_to_completion.py`
...
## T17: Agent 工具实现**文件:** `src/micodeagent/agent/agent_tool.py`
...
1. 新建文件,声明:
   ...
   ```
2. **解决循环依赖**:agent 包要引用 subagent 包,但 subagent 不应反过来。检查 `subagent.Definition` 是否引用 agent 包——目前 `Definition` 只引用 `permission` 包,没问题。直接 `from micodeagent.subagent import Definition`(或用 Protocol 解耦)
3. **AgentTool 接口实现**:
   - `name` 属性 = `"Agent"`
...
6. 提供 `set_parent(self, ag: "Agent") -> None` 让 cli 在 `MiCodeAgentApp` 构造之后回填 parent 引用
...
## T19: task 包基础结构**文件:** `src/micodeagent/task/manager.py`

**依赖:** T10, T15
**步骤:**

1. 新建包 `micodeagent.task`,加 `__init__.py` 与 `manager.py`
...
**验证:** `python -c "from micodeagent.task.manager import Manager"` 导入无误

## T20: Manager.launch 实现**文件:** `src/micodeagent/task/manager.py`
...
## T21: Manager.stop / adopt_running / send_message / upgrade_approval**文件:** `src/micodeagent/task/manager.py`
...
## T23: 4 个后台任务工具**文件:** `src/micodeagent/task/tools.py`
...
## T25: TUI 加 task_mgr / subagent_catalog wiring**文件:** `src/micodeagent/tui/app.py`

**依赖:** T6, T19, T23
**步骤:**

1. 在 `MiCodeAgentApp.__init__` 加形参:
   ```python
...
**验证:** `python -c "from micodeagent.tui.app import MiCodeAgentApp"` 导入无误

## T26: task notification 注入**文件:** `src/micodeagent/tui/tasks.py`
...
4. 把方法 `_consume_task_done` 挂到 `MiCodeAgentApp` 上(单独文件模块化,然后在 `app.py` `from .tasks import consume_task_done`)

**验证:** `python -c "from micodeagent.tui.tasks import build_task_notification"` 导入无误

## T27: ESC 切后台**文件:** `src/micodeagent/tui/stream.py`

**依赖:** T19, T25
**步骤:**

1. 在 `MiCodeAgentApp` 类挂 Textual binding:
   ```python
   ...
   ```
2. 增加 `foreground_sub_agent` 字段到 `MiCodeAgentApp` 跟踪当前前台子 Agent;Agent 工具开始前台跑动时设置,跑完清除
...
**验证:** `python -c "import micodeagent.tui.stream"` 导入无误

## T28: Skill fork 改造**文件:** `src/micodeagent/tui/skill_fork.py`
...
## T29: Agent 工具注册到 registry**文件:** `src/micodeagent/cli.py`
...
1. 在 `cli.main` 适当位置(`skills.load_catalog` 之后):
   ...
   ```
2. `MiCodeAgentApp(...)` 构造时传入 `task_mgr` / `subagent_catalog`:
   ```python
   app = MiCodeAgentApp(
       ...,
       writer=writer,
   ...
   ```
3. `MiCodeAgentApp` 构造后回填 parent:
   ```python
   if app.main_agent is not None:
       agent_tool.set_parent(app.main_agent)
   ```
4. `MiCodeAgentApp` 加 `main_agent` 属性返回 `self.agent`

**验证:** `python -m micodeagent --help` 不报错;`ruff check src/micodeagent/cli.py` 无告警

## T30: config 加 enable_subagent_background**文件:** `src/micodeagent/config.py`
...
## T31: agent.launch_fork 公用 wiring**文件:** `src/micodeagent/agent/launch.py`
...
4. **最终方案**(在文件结构里也已对齐):
   - `Definition` 类型放在 `micodeagent.subagent`
   - `launch_fork` 放在 `micodeagent.agent`(因为它要构造 `Agent`)
   - `AgentTool` 也放 `micodeagent.agent`(已有)
   - `tui/skill_fork.py` 调 `micodeagent.agent.launch_fork`(把 `Definition` 或裸参数传入)

**重新调整文件结构:**

- 删除 `src/micodeagent/subagent/launch.py`(本任务取消)
- 新建 `src/micodeagent/agent/launch.py` 实现 `launch_fork`
...
1. `uv sync`(确保依赖装好)
2. `ruff check src/micodeagent/`
3. `ruff format --check src/micodeagent/`
4. `mypy src/micodeagent/`(可选)
5. `pytest tests/ -v`
