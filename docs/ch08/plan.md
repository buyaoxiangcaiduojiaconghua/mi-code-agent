...
ch08 引入一个新的本地子包 `micodeagent.compact`，作为上下文管理的唯一权威入口。包内承担三块职责：

1. **第 1 层预防性压缩**：在每一轮 LLM 请求发出之前，对 `micodeagent.conversation` 中的工具结果做幂等的"超阈值落盘 + 字符串替换"，并把替换决策冻结在一个会话级账本里，保证 prompt cache 前缀逐字节稳定。
...
`micodeagent.compact` 不直接持有 `Agent`,也不直接管理 `Provider`。它通过一组窄接口与外部模块交互：

| 外部模块 | 交互方向 | 形式 |
|----------|----------|------|
| `micodeagent.agent` | Agent 调 compact | 主循环每轮请求前调 `manage_context`；ReadFile 成功后调 `RecoveryState.record_file`；捕获 `PromptTooLongError` 后调 `force_compact` 重试一次 |
| `micodeagent.conversation` | compact 改 conversation | compact 拿到 `list[Message]` 后做字符串替换 / 摘要重建，再用一个新方法 `replace_history` 整体替换内存列表 |
| `micodeagent.llm` | compact 调 provider | 摘要请求复用同一份 `Provider.stream`，但 `Request.tools` 留空；从 `StreamEvent` 尾部拿 usage 锚定 token 估算 |
| `micodeagent.tui` | TUI 调 compact | TUI 拿到以 `/` 开头的输入走命令分发；`/compact` 命令调 compact 的 `force_compact` 并展示 token 变化系统消息 |
| `micodeagent.config` | config 喂 compact | `ProviderConfig` 新增 `context_window: int`，未配置时按协议给默认值；compact 通过参数拿到当前 provider 的 `context_window` |

**Agent 生命周期与状态归属调整**：现状的 TUI 在 `_begin_turn` 里每轮 `Agent(...).run(...)` 重新构造一次 Agent（见 `src/micodeagent/tui/stream.py`），意味着把 compact 的长生命周期状态（替换决策账本、文件追踪、自动摘要熔断计数、`usage_anchor`、本轮工具列表缓存）放成 `Agent` 字段会被每轮重置——决策冻结与熔断器立刻失效。

本章引入 `SessionRuntime` 作为 TUI Model 跨 run 持有的长生命周期状态容器：

```python
# src/micodeagent/agent/runtime.py（新建）
...
**依赖方向无环**：
- `micodeagent.compact` 不 import `micodeagent.agent` / `micodeagent.config` / `micodeagent.tui` / `micodeagent.cli`。
- `micodeagent.config` 仅在 `effective_context_window()` 中读自身常量（`DEFAULT_ANTHROPIC_CONTEXT_WINDOW` / `DEFAULT_OPENAI_CONTEXT_WINDOW` 定义在 `src/micodeagent/config/protocol_defaults.py`，不放 compact 子包）。
- `micodeagent.agent` 依赖 `micodeagent.compact` + `micodeagent.conversation` + `micodeagent.llm` + `micodeagent.tool` + `micodeagent.permission`，**不** import `micodeagent.config`。
- `src/micodeagent/cli.py` 是唯一同时 import `config` 与 `agent` 的位置，负责把 `provider_cfg.effective_context_window()` 注入 `SessionRuntime`。
- `micodeagent.tui` 持有 `SessionRuntime` 与 `Agent`（或在每轮构造 Agent 时把 runtime 传入）。

## 核心数据结构

```python
# src/micodeagent/compact/state.py
...
# spill_dir 是落盘目录，固定指向 .micodeagent/sessions/<session_id>/tool-results/。
@dataclass
...
```python
# src/micodeagent/compact/const.py
...
```python
# src/micodeagent/config/config.py 改动(仅追加字段,不动现有字段顺序与名字)
...
# src/micodeagent/config/protocol_defaults.py（新文件）
...
> **依赖方向说明**：协议默认值常量定义在 `src/micodeagent/config/protocol_defaults.py`，由 `micodeagent.config` 自身使用；`micodeagent.compact` 包不持有协议默认值常量。`config` 与 `compact` 单向无环。
...
def offload_and_snip(
    ...
    (micodeagent 在 Conversation.add_tool_results 把一轮工具结果挂在一条 RoleTool
...
依赖：`micodeagent.llm.Provider`、`summary_prompt`、`recovery`、`token`、`CompactCircuitBreaker`。
...
依赖：`FileReadRecord`、`micodeagent.llm.ToolDefinition`。
...
### Agent 主循环改造（`src/micodeagent/agent/agent.py`）
...
3. **`_stream_once` 签名扩展为返回 (text, calls, usage, err)**：把现有 `(text, calls, usage, ok)` 改成 `(text, calls, usage, err: Exception | None)`。错误来源是 `StreamEvent.err`（micodeagent 的 `Provider.stream` 是 async generator，错误通过事件流投递）。`_stream_once` 在收到 ev.err 时累加的 text 不写回 Conversation（保证 Conversation 状态与 stream 调用前一致，紧急压缩可以安全地 `replace_history`）。
...
> **压缩状态事件 emit**（兑现 spec F24a / F24b）：`Event` 结构新增 `compact: CompactEvent | None` 字段（`src/micodeagent/agent/event.py`）。主循环在以下两个时机向事件队列 emit 状态事件，让 TUI 在 LLM 摘要请求还在跑的时候就能立刻显示"压缩中"前缀，避免用户以为程序卡死：
>
...
>
> ```python
> # src/micodeagent/agent/event.py
> from dataclasses import dataclass
> from enum import Enum
>
...
### Conversation 改造（`src/micodeagent/conversation.py`）
...
### TUI 命令分发（`src/micodeagent/tui/`）

`src/micodeagent/tui/stream.py` 现有 `submit()` 内部已经有针对 `/exit` / `/plan` / `/do` 的 `match/case` 分支。本章把这三个命令一并迁移到统一注册表，并新增 `/compact`：

```python
# src/micodeagent/tui/commands.py(新文件)

from collections.abc import Awaitable, Callable
from typing import Any

CommandHandler = Callable[[Any], Awaitable[None]]   # 入参为 MiCodeAgentApp
...
async def handle_compact(app: "MiCodeAgentApp") -> None:
...
**TUI 渲染 Compact 事件**（兑现 spec F24a / F24b）：`src/micodeagent/tui/stream.py` 的 `_update_streaming` 在 `StreamMsg` 处理上新增 `msg.compact is not None` 分支，按 phase 渲染系统消息后继续 `await self.events.__anext__()` 拉下一帧，**不写入 conversation**：
...
### config 改造（`src/micodeagent/config/`）
...
### `.micodeagent/config.yaml.example` 更新
...
`src/micodeagent/agent/agent.py` 改动：
...
`src/micodeagent/agent/runtime.py`（新文件）：定义 `SessionRuntime` dataclass 与构造工厂。

`src/micodeagent/agent/event.py`（新文件或修改 `__init__.py`）：定义 `CompactPhase`、`CompactEvent`；`Event` dataclass 追加 `compact: CompactEvent | None = None`。
...
`src/micodeagent/tui/commands.py`（新文件）：`dispatch_command` + `handle_exit` / `handle_plan` / `handle_do` / `handle_compact` + 未知命令兜底。
`src/micodeagent/tui/stream.py`：`submit()` 内原 `match/case` 改用 `dispatch_command` 调用；命令路径不调 `conv.add_user`，不写入对话历史。
`src/micodeagent/tui/app.py`：`MiCodeAgentApp` 新增 `runtime: SessionRuntime` 与 `agent: Agent` 字段；`__init__` 构造期一次性构造 Agent 并保存。
`tests/tui/test_tui.py`：① `/compact` 走命令路径不发 LLM；② `/unknown` 友好提示；③ 迁移后 `/exit` / `/plan` / `/do` 行为不回归三个用例。

`src/micodeagent/config/config.py`：`ProviderConfig` 追加 `context_window: int = 0`，加 `effective_context_window(p)`；现有字段顺序与 yaml 键名不变。
`src/micodeagent/config/protocol_defaults.py`（新文件）：`DEFAULT_ANTHROPIC_CONTEXT_WINDOW = 200000`、`DEFAULT_OPENAI_CONTEXT_WINDOW = 128000`。
`tests/test_config.py`：新增四种情况断言。

`src/micodeagent/cli.py`：启动阶段调 `new_session_context(workspace)`、`ContentReplacementState()`、`RecoveryState()`、`CompactCircuitBreaker()`，组装为 `SessionRuntime`；待 provider 选定后注入 `effective_context_window(p)`；把 `SessionRuntime` 交给 `MiCodeAgentApp`。

`scripts/smoke.py`（若存在）：同样按新签名构造 Agent；smoke 场景的 `context_window` 可固定 200000。

`.gitignore`：追加 `.micodeagent/sessions/`，避免开发者跑一次 micodeagent 后 `git status` 出现一大坨 session 子目录。

`.micodeagent/config.yaml.example`：新增 `context_window` 字段示例与注释。
...
| 决策点                                                      | 选择                                                                             | 理由                                                                                                                                                                                                                                                                                                                                         |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 包结构与命名                                                | `micodeagent.compact` 单子包，子模块按职责拆分（layer1 / layer2 / recovery / token） | 上下文管理逻辑高度内聚，对外只暴露 `manage_context` 等少量函数。单子包简化导入路径，多模块保证可读性。再拆 sub-sub-package 会引入循环引用风险（layer2 既要 token 又要 recovery）。                                                                                                                                                           |
...
| ReadFile 后用 `Path.read_bytes` 重读纯净字节                | 工具 worker task 内同步 `await asyncio.to_thread(...)`                           | 工具返回字符串带行号前缀（micodeagent 现有实现），直接拿来做恢复段会让模型把行号当成代码的一部分。重读一次磁盘成本可忽略；同步顺序保证下一次 `manage_context` 能观察到本轮记录。                                                                                                                                                                 |
...
| `session_id` 不持久化                                       | 进程启动生成 `<unix_ts>-<short_random>`（`secrets.token_hex(4)`）                | 单进程会话边界等于进程边界，不需要恢复。`.micodeagent/sessions/` 留作调试副产物，外部脚本/用户决定清理时机。                                                                                                                                                                                                                                     |
...
| `context_window` 注入时机                                   | provider 选定后由`micodeagent.cli` 注入 `SessionRuntime`，本会话内不变               | micodeagent 启动期 TUI 可能选 provider，等用户选定后才能确定`context_window`；本章不支持运行期切 provider；切换 provider 等同于重新启动进程。                                                                                                                                                                                                    |
