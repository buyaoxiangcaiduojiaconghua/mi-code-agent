...
- [ ] **C1**：`src/micodeagent/compact/` 子包存在，可被其他模块 import。
  - 验证：`ls src/micodeagent/compact/` 列出 `compact.py` / `layer1.py` / `layer2.py` / `summary_prompt.py` / `recovery.py` / `token.py` / `state.py` / `const.py` / `__init__.py` 九个核心文件，外加 `tests/compact/` 下的 `test_*.py`。
  - 验证：`python -c "import micodeagent.compact"` 退出码 0，无 ImportError。

- [ ] **C2**：常量集中在 `const.py`，未散落到其他文件。
  - 验证：在 `src/micodeagent/compact/` 内执行 `grep -rn "= 50000\|= 200000\|= 20000\|= 13000\|= 3000\|= 10000"` 命中点全部在 `const.py`。
  - 验证：`DEFAULT_ANTHROPIC_CONTEXT_WINDOW` 与 `DEFAULT_OPENAI_CONTEXT_WINDOW` 是 `src/micodeagent/config/protocol_defaults.py` 中的导出常量，可被 `micodeagent.config` 引用。
...
- [ ] **C16**：`llm.PromptTooLongError` 哨兵异常存在并被 provider 包装。
  - 验证：`grep -n PromptTooLongError src/micodeagent/llm/__init__.py` 命中。
...
- [ ] **I7**：TUI 命令分发表注册四项（迁移现有 `/exit` / `/plan` / `/do` + 新增 `/compact`）。
  - 验证：`grep -n "/compact\|/exit\|/plan\|/do" src/micodeagent/tui/commands.py` 命中四项；`BUILTIN_COMMANDS` 字典长度为 4。
...
- [ ] **I9**：`src/micodeagent/cli.py` 启动时把 `effective_context_window(p)` 注入到 Agent。
  - 验证：跑 `python -m micodeagent` 并配置 anthropic provider 不带 `context_window` → Agent 字段拿到 200000。
  - 验证：把 `context_window: 100000` 加入配置 → Agent 字段拿到 100000。

- [ ] **I10**：`.micodeagent/config.yaml.example` 展示新字段用法与默认值注释。
  - 验证：打开示例文件，看到 `context_window: 200000` 之类的字段和 "可选；未配置时按 protocol 默认" 注释。

### 会话目录

- [ ] **I11**：进程启动后 `.micodeagent/sessions/<id>/tool-results/` 物理目录被创建。
  - 验证：启动 micodeagent 后 `ls .micodeagent/sessions/` 出现新子目录；子目录名形如 `<unix_ts>-<hex>`。
...
- [ ] **B1**：`python -m compileall src/micodeagent` 退出码 0，无语法错误；`python -c "import micodeagent"` 成功。

- [ ] **B2**：`ruff check src/micodeagent/` 无告警。

- [ ] **B3**：`ruff format --check src/micodeagent/compact/ src/micodeagent/agent/ src/micodeagent/conversation.py src/micodeagent/llm/ src/micodeagent/tui/ src/micodeagent/config/ src/micodeagent/cli.py` 输出为空（全部已格式化）。

- [ ] **B4**：`ruff check --select I src/micodeagent/compact/` 输出为空；import 分组遵循"标准库 / 第三方 / 本地"三段，组间空行隔开（PEP 8 + isort 默认风格）。
...
- [ ] **B11**：注释不出现"参考"、"取自"、"对齐"、"mirror"、"镜像"、"TS 实现"、"Go 实现"等外部引用语。
  - 验证：`grep -rn "参考\|取自\|对齐.*实现\|mirror\|镜像\|TS 实现\|TypeScript 实现\|Go 实现\|as in\|课程实现\|README" src/micodeagent/compact/ src/micodeagent/agent/ src/micodeagent/conversation.py src/micodeagent/llm/ src/micodeagent/tui/ src/micodeagent/config/` 全部无命中。
...
- [ ] **B12**：`.micodeagent/config.yaml.example` 可被解析。
  - 验证：编写一个测试 fixture，`yaml.safe_load(Path(".micodeagent/config.yaml.example").read_text())` 不报错；映射到 `Config` 后 `validate()` 通过；新增的 `context_window` 字段在解码后非零。
...
- [ ] **预期**：下一轮 stream 请求 messages 中该工具结果 content 已被替换为预览体，通过 4 条 `in` 断言：① 包含 `original size:` 子串与字节数（"80000"）；② 包含 `[saved to]` 与 `spill_dir` 尾段路径片段；③ 包含 `head preview` 标记；④ 包含 `文件读取工具` 与 `不要凭头部预览猜测` 两个关键短语。`.micodeagent/sessions/<id>/tool-results/<tool_use_id>` 文件存在且 size = 80000 字节。
...
- [ ] **触发**：`uv sync` 或 `pip install -e .`，在 tmux 中启动 `python -m micodeagent`，配置 anthropic provider。
- [ ] **预期**：
  - 让 Agent 读一个 80KB 的本地文件 → `.micodeagent/sessions/<id>/tool-results/` 下出现该工具调用 id 的文件；
  ...
  - 进程退出后 `.micodeagent/sessions/<id>/` 仍存在，下次启动再开新子目录。
- [ ] **观察方式**：tmux 中目测；用 `ls .micodeagent/sessions/` 与 `cat` 抽查落盘文件；`git status` 干净（覆盖 I12 `.gitignore`）。
