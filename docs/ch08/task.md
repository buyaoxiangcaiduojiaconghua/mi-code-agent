...
| 文件路径 | 类型 | 职责 |
|----------|------|------|
| `src/micodeagent/compact/__init__.py` | 新建 | 重导出 `manage_context` / `TriggerKind` / 几个 State 类型 |
| `src/micodeagent/compact/const.py` | 新建 | 全部硬编码常量 |
| `src/micodeagent/compact/state.py` | 新建 | `ContentReplacementState`（含 `decide_once`）/ `CompactCircuitBreaker` / `RecoveryState` / `SessionContext` |
| `src/micodeagent/compact/token.py` | 新建 | `estimate_tokens` / `usage_anchor` / `message_chars` |
| `src/micodeagent/compact/layer1.py` | 新建 | `offload_and_snip` / `spill_single` / `build_preview` |
| `src/micodeagent/compact/summary_prompt.py` | 新建 | `build_summary_prompt` / `serialize_conversation` / `extract_summary` / 9 部分模板 |
| `src/micodeagent/compact/recovery.py` | 新建 | `build_recovery_attachment` / `render_file_block` / `render_tools_block` / `BOUNDARY_NOTICE` |
| `src/micodeagent/compact/layer2.py` | 新建 | `auto_compact` / `force_compact` / `run_summary` / `summarize_once` / `ptl_retry` / `pick_recent_tail` / `group_by_user_turn` |
| `src/micodeagent/compact/compact.py` | 新建 | `manage_context` / `TriggerKind` 枚举 / 编排 |
| `tests/compact/test_*.py` | 新建 | 各模块对应单测 |
| `src/micodeagent/llm/__init__.py` | 修改 | 新增 `class PromptTooLongError(Exception)` 哨兵异常（`ToolDefinition` 已是导出类型，无需改动） |
| `src/micodeagent/llm/anthropic_provider.py` | 修改 | 把 provider 上下文过长异常包装成 `PromptTooLongError` 并通过 `yield StreamEvent(err=...)` 投递 |
| `src/micodeagent/llm/openai_provider.py` | 修改 | 同上 |
| `tests/llm/test_anthropic_provider.py` | 修改/新建 | PTL 异常包装单测 |
| `tests/llm/test_openai_provider.py` | 修改/新建 | PTL 异常包装单测 |
| `src/micodeagent/conversation.py` | 修改 | 加 `# 无需显式锁`；新增 `replace_history(new_messages)` 深拷贝整体替换 |
| `tests/test_conversation.py` | 修改 | 增加 `replace_history` 用例 |
| `src/micodeagent/config/config.py` | 修改 | `ProviderConfig` 追加 `context_window: int = 0` + 模块级函数 `effective_context_window(p)` |
| `src/micodeagent/config/protocol_defaults.py` | 新建 | `DEFAULT_ANTHROPIC_CONTEXT_WINDOW` / `DEFAULT_OPENAI_CONTEXT_WINDOW` 协议默认值常量 |
| `tests/test_config.py` | 修改 | 4 种情况断言 + `yaml.safe_load` `.micodeagent/config.yaml.example` 通过的解析测试 |
| `src/micodeagent/agent/runtime.py` | 新建 | `SessionRuntime` dataclass |
| `src/micodeagent/agent/event.py` | 新建 | `CompactPhase` / `CompactEvent`；`Event` dataclass 追加 `compact` 字段 |
| `src/micodeagent/agent/agent.py` | 修改 | `__init__` 接受 `runtime` 关键字参数；`_stream_once` 签名改为返回 err；主循环集成 compact、ReadFile 追踪、PTL 紧急压缩、`run_force_compact`、`_run_lock` 互斥锁 |
| `tests/agent/test_agent.py` | 修改 | fake_provider 扩展 + 紧急压缩两用例 |
| `src/micodeagent/tui/commands.py` | 新建 | 命令分发 + `/exit` / `/plan` / `/do` / `/compact` 处理器 + 未知命令兜底 |
| `src/micodeagent/tui/app.py` | 修改 | `MiCodeAgentApp` 新增 `runtime: SessionRuntime` 与 `agent: Agent` 字段；`__init__` 构造期一次性构造 Agent |
| `src/micodeagent/tui/stream.py` | 修改 | `submit()` 内原 match 改用 `dispatch_command` |
| `tests/tui/test_tui.py` | 修改 | 5 组用例:`/compact` 路由到命令分发、`/unknown` 友好提示、迁移后 `/exit` / `/plan` / `/do` 各一组不回归 |
| `src/micodeagent/cli.py` | 修改 | 启动期构造 `SessionRuntime` 注入 TUI；待 provider 选定后再注入 `context_window` |
| `scripts/smoke.py` | 修改 | 按新 Agent 构造签名传入 `SessionRuntime`（smoke 场景 `context_window=200000`） |
| `.micodeagent/config.yaml.example` | 修改 | 新增 `context_window` 字段示例与注释 |
| `.gitignore` | 修改 | 追加 `.micodeagent/sessions/` |

---

## T1 - 建立 compact 子包骨架与常量

- **文件**：`src/micodeagent/compact/__init__.py`、`src/micodeagent/compact/const.py`
- **依赖**：无
- **步骤**：
  1. 新建目录 `src/micodeagent/compact/`，添加空 `__init__.py`（后续按导出需求填充 `from .compact import manage_context, TriggerKind` 等）。
...
- **验证**：`python -c "from micodeagent.compact import const"` 不报错；`ruff check src/micodeagent/compact/` 无告警。

## T2 - SessionContext 与目录创建

- **文件**：`src/micodeagent/compact/state.py`
- **依赖**：T1
- **步骤**：
  ...
  4. 实现 `new_session_context(workspace: str) -> SessionContext`：
     ...
     - 拼接 `spill_dir = str(Path(workspace) / ".micodeagent/sessions" / session_id / "tool-results")`。
     - 调 `Path(spill_dir).mkdir(parents=True, exist_ok=True)`；目录已存在不算错误。
     - 返回构造好的实例。
  5. 文件头部 import 按 `ruff format` / `isort` 规则分组（标准库、第三方、本地）。
- **验证**：`python -c "from micodeagent.compact.state import new_session_context; print(new_session_context('.'))"` 输出合理；T22 增加 `test_new_session_context_rand_fail_fallback`（通过 monkeypatch 注入 `secrets.token_hex` 失败模拟）覆盖降级路径。
...
- **验证**：`ruff check src/micodeagent/compact/state.py` 干净；目测逻辑正确无并发冲突；`decide_once` 是临界区入口。
...
- **文件**：`src/micodeagent/compact/token.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/micodeagent/compact/layer1.py`
- **依赖**：T2
- **步骤**：
...
- **验证**：`ruff check src/micodeagent/compact/layer1.py` 干净；手工写一个脚本测一次重复落盘，第二次 `st_mtime_ns` 不变（也可留到 T22 测试覆盖）。
...
- **步骤**：
  ...
  3. 关键约束：仅遍历 `msg.role == "tool"` 的消息（micodeagent 把一轮工具结果挂在 tool 消息的 `tool_results` 列表里，**不**在 assistant 消息里）。对每条 tool 消息独立处理其 `tool_results` 列表。
...
- **文件**：`src/micodeagent/compact/summary_prompt.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/micodeagent/compact/recovery.py`
- **依赖**：T4
- **步骤**：
  1. 新建 `recovery.py`。
...
- **文件**：`src/micodeagent/compact/layer2.py`
- **依赖**：T5
- **步骤**：
  1. 新建 `layer2.py`。
...
- **文件**：`src/micodeagent/compact/compact.py`
- **依赖**：T8、T17
- **步骤**：
  1. 新建 `compact.py`。
...
- **验证**：`ruff check src/micodeagent/compact/` 干净；`pytest tests/compact/test_compact.py` 通过。

## T19 - llm.PromptTooLongError 哨兵

- **文件**：`src/micodeagent/llm/__init__.py`
- **依赖**：无
- **步骤**：
...
- **验证**：`python -c "from micodeagent.llm import PromptTooLongError"` 不报错；`grep -n PromptTooLongError src/micodeagent/llm/__init__.py` 命中。

## T20 - Anthropic / OpenAI provider PTL 错误包装

- **文件**：`src/micodeagent/llm/anthropic_provider.py` / `src/micodeagent/llm/openai_provider.py`
- **依赖**：T19
- **步骤**：
...
- **验证**：`ruff check src/micodeagent/llm/` 干净。
...
- **文件**：`src/micodeagent/conversation.py`
- **依赖**：无
- **步骤**：
...
- **验证**：`ruff check src/micodeagent/conversation.py` 干净；T23 加并发用例覆盖 `_lock`。
...
- **依赖**：T1~T18
- **步骤**：
  1. **conftest.py**（pytest fixture 文件）：定义 `fake_compact_provider` fixture——脚本化驱动 `list[list[StreamEvent]]`，支持按调用次数 yield 不同异常（覆盖 `PromptTooLongError` 与其他异常）；在最后一帧之前 yield Usage 事件。compact 包内部测试统一用这个 helper。**不要** import `micodeagent.agent` 的 fake_provider（位于 `tests/agent/`，作用域不重合）。
  2. `test_state.py`：
...
- **文件**：`src/micodeagent/config/config.py` + `src/micodeagent/config/protocol_defaults.py`（新建）
- **依赖**：T1
- **步骤**：
  1. 新建 `src/micodeagent/config/protocol_defaults.py`：
...
- **验证**：`ruff check src/micodeagent/config/` 干净；`grep -n compact src/micodeagent/config/` 无命中（config 不 import compact）。
...
- **文件**：`src/micodeagent/agent/runtime.py`（新建）+ `src/micodeagent/agent/agent.py`（修改）
- **依赖**：T2、T3、T4、T19、T24
- **步骤**：
  1. 新建 `src/micodeagent/agent/runtime.py`：
...
- **验证**：`ruff check src/micodeagent/agent/` 干净。

## T27 - Agent 主循环集成 manage_context

- **文件**：`src/micodeagent/agent/agent.py`
- **依赖**：T18、T21、T26
- **步骤**：
...
- **步骤**：
  ...
  2. 判断条件：`calls[i].name == "read_file"`（micodeagent 现有工具名以 `src/micodeagent/tool/read_file.py` 的 `name` 属性为准）且 `tool.Result.is_error is False`（`is_error` 来自 `tool.Result`，不是 `llm.ToolResult`）。
  3. 从 `ToolCall.input`（dict）取参数：
     ...
     注意 `ToolCall` 字段在 Python 适配后是 `input` 而非 `arguments`，参数名以 `src/micodeagent/tool/read_file.py` 定义的 schema 为准（micodeagent 现在用 `path`）。
...
- **验证**：`ruff check src/micodeagent/agent/` 干净；自查只对 ReadFile 工具触发，其他工具不会写 recovery。
...
- **文件**：`src/micodeagent/agent/event.py`（新建）+ `src/micodeagent/agent/agent.py`、`tests/agent/test_agent.py`
- **依赖**：T27、T29
- **步骤**：
  1. 新建 `src/micodeagent/agent/event.py`：
...
- **文件**：`src/micodeagent/cli.py` + `scripts/smoke.py`
- **依赖**：T2、T3、T4、T26
- **步骤**：
  1. 启动阶段构造：
  ...
  2. `src/micodeagent/cli.py`：若 `providers` 长度 == 1，启动期即可知 protocol → 直接计算 `cw = effective_context_window(providers[0])`，构造 `runtime = SessionRuntime(...)` 注入 `MiCodeAgentApp`。若 `len(providers) > 1`，让 TUI 完成 provider 选择后再注入 `context_window`（在 TUI 的 provider-selected 回调里调 `runtime.context_window = effective_context_window(provider_cfg)`）。
...
- **验证**：`python -m micodeagent --help` 可启动；启动后 `.micodeagent/sessions/<id>/tool-results/` 目录存在。

## T32.5 - TUI Model 持有 SessionRuntime 与 Agent 引用

- **文件**：`src/micodeagent/tui/app.py` + `src/micodeagent/tui/stream.py`
- **依赖**：T26、T32
- **步骤**：
  1. `MiCodeAgentApp` 类新增字段 `runtime: SessionRuntime` 与 `agent: Agent`。
...
- **验证**：`ruff check src/micodeagent/tui/` 干净。

## T33 - TUI 命令分发框架

- **文件**：`src/micodeagent/tui/commands.py`
- **依赖**：T30、T32.5
- **步骤**：
  1. 新建 `commands.py`。
  2. 定义 `CommandHandler = Callable[["MiCodeAgentApp"], Awaitable[None]]`。
...
- **文件**：`src/micodeagent/tui/stream.py`
- **依赖**：T33
- **步骤**：
  1. micodeagent 现有 `src/micodeagent/tui/stream.py` 的 `submit()` 内部已有针对 `/exit` / `/plan` / `/do` 的 match 分支。把这段 match 整段删除，改为统一的命令分发：
...
- **验证**：`ruff check src/micodeagent/tui/` 干净；手动跑 `tmux` 启动 micodeagent 输入 `/compact`、`/exit`、`/plan`、`/do` 验证行为；输入 `/unknown` 看到友好提示。

## T34a - TUI 渲染 Compact 状态事件（兑现 spec F24a / F24b）

- **文件**：`src/micodeagent/tui/stream.py`、`src/micodeagent/tui/commands.py`、`tests/tui/test_tui.py`
- **依赖**：T29a、T34
- **步骤**：
...
- **文件**：`.micodeagent/config.yaml.example`
- **依赖**：T24
- **步骤**：
...
- **验证**：编写小测试 fixture，`yaml.safe_load(Path(".micodeagent/config.yaml.example").read_text())` 解析后通过 `_from_dict` 转为 `Config` 不报错。
...
- **依赖**：T2
- **步骤**：
  1. 在 `.gitignore` 末尾追加一行 `.micodeagent/sessions/`。
  2. 提交时在 PR 说明中点出。
- **验证**：启动 micodeagent 后跑一次工具调用，`git status` 干净（不出现 sessions 子目录）。
...
- **步骤**：
  ...
  2. 用 tmux 启动 `python -m micodeagent`，配置好 anthropic provider。
  3. 触发一次大文件 ReadFile（构造一个 80KB 文件）→ 观察 `.micodeagent/sessions/<id>/tool-results/` 下出现该工具调用 id 的文件；下一轮请求里该工具结果展示成预览体。
