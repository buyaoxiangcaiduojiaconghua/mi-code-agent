...
1. **权限匹配器升级层(`micodeagent.permission` 包内改造)**——把 Pattern 形态从字符串升级到结构化 `Matcher` Protocol;新增 exact/regex/not 三种实现,glob 保留作为缺省类型。改造对外仅暴露语法升级和 stderr 错误回退,运行时 Allow/Deny 语义不变。

2. **Hook 主体层(新建 `micodeagent.hook` 包)**——加载 YAML 规则、提供事件分派引擎、四类动作执行器;通过 11 个事件 emit 点接入 agent / tui。
...
cli.main 完成 wiring → tui.create_app 返回 MiCodeAgentApp → app.run_async()
│
└─ Textual on_mount() 末尾
首条 user 输入到达前
派发 SessionStart 事件
...
实际接入:`MiCodeAgentApp.on_mount()` 末尾 `await self._dispatch_session_start()`,该协程同步调 `Engine.dispatch`、收集 `injected_prompts` 注入到 `runtime.pending_reminders`、然后返回。
...
```python
# micodeagent/permission/matcher.py
...
**改动文件:** `src/micodeagent/permission/rule.py`(扩展 `parse_rule` / `match_rule`)、新增 `src/micodeagent/permission/matcher.py`。
...
**依赖:** 模块 D。
**改动文件:** `src/micodeagent/agent/agent.py`、`src/micodeagent/agent/runtime.py`(`SessionRuntime` 加 `pending_reminders: list[str]`、`reset_for_new_session` 清空)。
...
**对外接口:** `MiCodeAgentApp` 私有方法 `_dispatch_session_start` / `_dispatch_session_end` 等。
**依赖:** 模块 D。
**改动文件:** `src/micodeagent/tui/app.py`、`src/micodeagent/tui/stream.py`、`src/micodeagent/tui/commands.py`(/clear、/resume 触发 SessionEnd + SessionStart/Resume)。

### 模块 I:`/hooks` 命令**职责:** 输出已加载 hook 列表 + 加载来源文件。

**对外接口:** 注册到 `command.register_builtins`。
**依赖:** `MiCodeAgentApp` 实现 UI 接口暴露 `hook_sources()` / `hook_rules()` 查询方法。

### 模块 J:cli wiring**职责:** 在 `cli.main` 中调 `hook.load(project_root)`,把 Engine 注入 agent 与 App。

**改动文件:** `src/micodeagent/cli.py`、`src/micodeagent/tui/app.py`(`AppParams` 加 `hook_engine` 字段)。
...
| Hook 包独立                                     | `micodeagent.hook`                                | 与`micodeagent.permission` 平级;hook 依赖 `permission.Matcher`,但 permission 不依赖 hook,无循环                 |
