...
- [ ] `micodeagent.hook` 包存在且可导入(验证:`python -c "import micodeagent.hook"` 不抛 ImportError)
...
- [ ] `hook.Engine` 与 `permission.Matcher` 共用同一套匹配实现(验证:`micodeagent.hook` 包不重复实现 exact/regex/glob)
...
- [ ] 项目可导入无错误:`python -c "import micodeagent"`
- [ ] 入口可启动:`python -m micodeagent --help` 正常输出
- [ ] 所有单元测试通过:`pytest`
- [ ] ruff 检查通过:`ruff check src tests`
- [ ] (可选)类型检查:`mypy src/micodeagent/hook` 无 error

## 端到端场景(tmux 实跑)

每个场景在 tmux 内启动一个 micodeagent 实例完成,验证人工/可视化行为。

### 场景 1:PreToolUse shell 拦截 write_file**预置:** 在 `.micodeagent/hooks.yaml` 写一条 hook:
```yaml
hooks:
  - name: block-write
    event: PreToolUse
...
- [ ]  tmux 启动 micodeagent
- [ ]  micodeagent 启动期 stderr 打印 `hook "bad-async": async not allowed for blocking events, skipped`
- [ ]  micodeagent 仍然成功进入 idle 状态
