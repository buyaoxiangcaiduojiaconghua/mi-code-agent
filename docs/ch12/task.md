...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/permission/matcher.py` | Matcher Protocol、四种实现、`compile_matcher` 工厂 |
| 新建 | `tests/permission/test_matcher.py` | 四种 type × 边界条件覆盖 |
| 修改 | `src/micodeagent/permission/rule.py` | `parse_rule` 识别前缀、`Rule` 持有 `matcher` 替代 pattern 字符串、`hit_any` / `match_rule` 改造 |
| 修改 | `tests/permission/test_rule.py` | 扩展用例覆盖新语法 |
| 修改 | `src/micodeagent/permission/settings.py` | `to_rule_set` 改造:失败 rule 走 stderr |
| 修改 | `tests/permission/test_settings.py` | 验证 stderr 报错与跳过逻辑 |
| 新建 | `src/micodeagent/hook/__init__.py` | 包标识 + 暴露 `Engine` / `Event` / `load` / `DispatchResult` |
| 新建 | `src/micodeagent/hook/event.py` | 11 个 `Event` 枚举 + 拦截类列表 + `is_blocking` 判定 |
| 新建 | `src/micodeagent/hook/rule.py` | `Rule` / `Condition` / `Action` / `Payload` 数据结构 |
| 新建 | `src/micodeagent/hook/matcher.py` | `eval_condition` / `get_by_path` |
| 新建 | `src/micodeagent/hook/loader.py` | YAML 解析、双层合并、字段校验 |
| 新建 | `tests/hook/test_loader.py` | 字段校验、加载错误、合并测试 |
| 新建 | `src/micodeagent/hook/engine.py` | `Engine` + dispatch 主流程 + only_once |
| 新建 | `tests/hook/test_engine.py` | 各事件 dispatch、拦截、reminder、once 覆盖 |
| 新建 | `src/micodeagent/hook/executor.py` | 四类 action 执行器 |
| 新建 | `tests/hook/test_executor.py` | shell exit2、http block、prompt、subagent stub |
| 修改 | `src/micodeagent/agent/runtime.py` | `SessionRuntime` 加 `pending_reminders` + `hook_engine` 字段 + `reset_for_new_session` 清空 |
| 修改 | `tests/agent/test_runtime.py` | 验证 `pending_reminders` 行为 |
| 修改 | `src/micodeagent/agent/agent.py` | `Agent.__init__(..., hook_engine=...)` 参数、11 个 emit 点(部分由 tui 触发,agent 负责 PreUserMessage/PreToolUse/PostToolUse/PreCompact/PostCompact/Stop/Notification) |
| 修改 | `tests/agent/test_agent.py` | 拦截路径测试 |
| 新建 | `src/micodeagent/tui/hooks.py` | `/hooks` 命令 handler、App 的 hook 查询方法 |
| 修改 | `src/micodeagent/tui/app.py` | `AppParams` 加 `hook_engine`、App 持有;`on_mount` 触发 SessionStart |
| 修改 | `src/micodeagent/tui/stream.py` | `_submit()` 内 UserPromptSubmit dispatch + 拦截集成 |
| 修改 | `src/micodeagent/tui/commands.py` | `/clear`、`/resume` 触发 SessionEnd + SessionStart/Resume |
| 修改 | `src/micodeagent/command/builtins.py` | 加 `/hooks` 内置命令 |
| 修改 | `src/micodeagent/command/ui.py` | UI 接口加 hook 查询方法 |
| 修改 | `src/micodeagent/cli.py` | 加 `hook.load(root)` 与 wiring;SessionEnd 兜底 |
| 修改 | `pyproject.toml` | 新增依赖 `httpx`(若未引入) |

## T1: 实现 `permission.Matcher` 接口与四种类型**文件:** `src/micodeagent/permission/matcher.py`
**依赖:** 无
**步骤:**
...
**验证:** `python -c "from micodeagent.permission.matcher import compile_matcher; print(compile_matcher('=foo', is_command=False))"` 输出 `=foo`
...
## T3: 升级 `permission.Rule` 与 `parse_rule`**文件:** `src/micodeagent/permission/rule.py`
**依赖:** T1
**步骤:**
...
## T4: 升级 `to_rule_set` 错误日志**文件:** `src/micodeagent/permission/settings.py`
**依赖:** T3
**步骤:**
...
**验证:** `python -c "from micodeagent.permission.settings import to_rule_set; print('ok')"` 不报错
...
## T6: hook 包基础数据结构**文件:** `src/micodeagent/hook/__init__.py`、`src/micodeagent/hook/event.py`、`src/micodeagent/hook/rule.py`
**依赖:** 无
**步骤:**
...
**验证:** `python -c "from micodeagent.hook import Event; print(Event.PRE_TOOL_USE.value)"` 输出 `PreToolUse`

## T7: `hook.matcher` 字段路径求值**文件:** `src/micodeagent/hook/matcher.py`
**依赖:** T6、T1
**步骤:**
...
## T8: `hook.Loader` YAML 解析**文件:** `src/micodeagent/hook/loader.py`
**依赖:** T6、T7、T1
**步骤:**
1. 定义 `def load(project_root: str | Path) -> Engine`:
   - 计算两个候选路径:`<project_root>/.micodeagent/hooks.yaml`、`Path.home() / ".micodeagent" / "hooks.yaml"`
...
**验证:** `python -c "from micodeagent.hook.loader import load; print(load('.'))"` 不抛异常
...
## T10: `hook.Engine` 与 dispatch 主流程**文件:** `src/micodeagent/hook/engine.py`
**依赖:** T6、T7
**步骤:**
...
**验证:** `python -c "import asyncio; from micodeagent.hook.engine import Engine; asyncio.run(Engine([], []).dispatch('Stop', {}))"` 通过(传字面量会失败,改用 Event.STOP)

## T11: `hook.Executor` 四类动作执行**文件:** `src/micodeagent/hook/executor.py`
**依赖:** T6
**步骤:**
...
**验证:** `python -c "from micodeagent.hook.executor import Executor; print(Executor())"` 通过
...
## T14: agent `SessionRuntime` 扩展**文件:** `src/micodeagent/agent/runtime.py`、`tests/agent/test_runtime.py`
**依赖:** T6
**步骤:**
...
## T15: `Agent.__init__` 加 hook_engine 与 emit 框架**文件:** `src/micodeagent/agent/runtime.py`、`src/micodeagent/agent/agent.py`
**依赖:** T14
**步骤:**
...
**验证:** `python -c "from micodeagent.agent.agent import Agent; print(Agent)"` 通过

## T16: agent 各事件 emit 接入**文件:** `src/micodeagent/agent/agent.py`
**依赖:** T15
**步骤:**
...
**验证:** `python -c "from micodeagent.agent.agent import Agent; print('ok')"` 通过
...
## T18: tui `MiCodeAgentApp` 持有 `hook_engine`**文件:** `src/micodeagent/tui/app.py`
**依赖:** T15
**步骤:**
1. `AppParams` dataclass 加 `hook_engine: Engine | None`
2. `MiCodeAgentApp` 类加属性 `self.hook_engine: Engine | None`
3. `__init__` 内:
...
**验证:** `python -c "from micodeagent.tui.app import MiCodeAgentApp; print(MiCodeAgentApp)"` 通过

## T19: tui `UserPromptSubmit` 拦截集成**文件:** `src/micodeagent/tui/stream.py`
**依赖:** T18
**步骤:**
1. `_submit()` 重写:
...
**验证:** `python -c "from micodeagent.tui.stream import ..."` 通过

## T20: tui SessionStart / End / Resume**文件:** `src/micodeagent/tui/app.py`、`src/micodeagent/tui/commands.py`、`src/micodeagent/tui/stream.py`
**依赖:** T18、T19
**步骤:**
...
**验证:** `python -c "from micodeagent.tui.app import MiCodeAgentApp; print('ok')"` 通过

## T21: `/hooks` 命令**文件:** `src/micodeagent/tui/hooks.py`、`src/micodeagent/command/builtins.py`、`src/micodeagent/command/ui.py`
**依赖:** T6、T10、T18
**步骤:**
1. UI Protocol 加方法 `hook_sources() -> list[str]`、`hook_rules() -> list[Rule]`
2. `MiCodeAgentApp` 实现这两个方法(读 `self.hook_engine` 属性,None 时返回空)
3. 新增 `src/micodeagent/tui/hooks.py`,实现 `async def handle_hooks(ctx, ui)`:
   - 取 rules 与 sources
...
## T22: `cli.main` wiring**文件:** `src/micodeagent/cli.py`
**依赖:** T8、T18
**步骤:**
...
兜底 SessionEnd
4. 顶部 `from micodeagent import hook`、`from micodeagent.hook import Event`

**验证:** `python -m micodeagent --help` 启动不报错
...
## T25: tmux 端到端实跑(验收 AC17 与 checklist 端到端场景)**文件:** `.micodeagent/hooks.yaml` 临时测试配置

**依赖:** T23、T24
**步骤:**

1. 写测试 `hooks.yaml`:包含 AC4-AC15 各典型场景的 hook
2. tmux 新建 session 启动 `python -m micodeagent` 或安装后的 `micodeagent`
