...
## 架构概览（分层）
1. 入口层 `micodeagent.cli` —— 加载配置、打印 banner、启动 Textual App。
2. 配置层 `micodeagent.config` —— 读取并校验 `.micodeagent/config.yaml`，给出 providers 列表。
3. LLM 协议层 `micodeagent.llm` —— 定义协议无关的 `Provider` Protocol 与统一消息/流式事件类型；
   anthropic、openai 两个适配器各自封装官方 SDK、统一吐出文本增量（思考增量内部丢弃）。
4. 会话层 `micodeagent.conversation` —— 进程内维护多轮历史，提供完整上下文。
5. 提示词/资源 `micodeagent.prompt` —— 内置 system prompt 与启动 banner（ASCII 猫）。
6. 终端层 `micodeagent.tui` —— Textual App，含状态机（选择/空闲/流式）、输入框、对话区、
...
## 模块设计### 模块 `micodeagent.config`

职责：读取并校验 `.micodeagent/config.yaml`，产出 providers 列表。
...
对外接口：`MiCodeAgentApp(providers: list[ProviderConfig])`；`App.run()` 或 `await App.run_async()`。
内部职责：
...
### 模块 `micodeagent.cli`（入口）

职责：装配与启动。
流程：`config.load(...)` → `prompt.render_banner(version, cwd)` 打印 → `MiCodeAgentApp(cfg.providers).run()`。
...
```
main() → config.load(".micodeagent/config.yaml")
       → 若 ConfigError：打印可读错误、sys.exit(1)
       → print(prompt.render_banner(version, cwd))
       → MiCodeAgentApp(cfg.providers).run()
...
Provider.stream ──async generator──> StreamEvent (text / done / err)
                                         │
                                         └──async for──> MiCodeAgentApp._consume_stream
                                                            │
                                                            ├── text  → 动态区追加
...
│   └── micodeagent/
│       ├── __init__.py
│       ├── __main__.py             — 允许 `python -m micodeagent`
...
- `.micodeagent/config.yaml` 含真实密钥，应在 `.gitignore` 忽略；提交一份 `config.yaml.example`。
- `pyproject.toml` 里通过 `[project.scripts] micodeagent = "micodeagent.cli:main"` 暴露 CLI 入口；
  装好后既可 `micodeagent` 也可 `python -m micodeagent`。
...
| 决策点        | 选择                                                                        | 理由                                                                                    |
| ------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| 语言          | Python 3.12+                                                                | 项目既定（micodeagent python 线）；3.12 的 typing/`asyncio.TaskGroup` 等更舒服              |
...
| 配置          | `.micodeagent/config.yaml` + `pyyaml`；密钥入 `.gitignore`                      | 用户既定路径；N5 密钥安全                                                               |
