...
- **tui 模块（改造）**：`MiCodeAgentApp.mode` 改为 `permission.Mode`，持有 `Engine`；新增 `Approving` 态与待批准请求渲染/按键处理；**全局 ctrl+c/esc 分派从仅 `Streaming` 扩展到 `Streaming | Approving`**（见下，否则 approving 态 ctrl+c 会退出整个程序）；**新增全局 `shift+tab` 按键循环切换权限模式**（仅 idle 态生效）；状态栏左侧改为**常驻显示当前权限模式（取代 provider 名）**；把会话/永久放行的规则写入交给引擎（经 agent 在 Loop 内应用，TUI 只回传用户选择）。
...
### tui 模块
```python
# 现有签名保持 (MiCodeAgentApp,) 不变，仅末尾增 engine 形参：
def new_app(
...
**关键点：**
- `MiCodeAgentApp`：`mode: permission.Mode`（初值 `engine.start_mode()`）；加 `engine: Engine`、`pending: ApprovalRequest | None`。
- `new_app(providers, version, registry, engine)`（保持返回 `MiCodeAgentApp`）：存引擎、置初始模式。
...
├── src/micodeagent/agent.py   — 改:删 Mode（迁 permission）;Agent 加 engine;execute_batched(+mode)接入 check;request_approval;ApprovalRequest 事件;Deny 用 ToolResult 构造
├── src/micodeagent/tui/
...
├── src/micodeagent/config.py  — 不改（provider 配置与 permission settings 分离）
├── src/micodeagent/cli.py     — 改:构造 permission.Engine 注入 tui
...
├── .gitignore             — 改:加 .micodeagent/settings.local.yaml
└── .micodeagent/settings.yaml.example — 新:权限配置示例（default_mode + allow/deny）
```
...
| 永久放行落点 | 写本地层 `.micodeagent/settings.local.yaml`（gitignore） | 用户拍板；不进 git、不影响队友（对齐 Claude Code don't-ask-again） |
