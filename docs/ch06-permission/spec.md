...
## 背景
ch05 做完系统提示工程化后，MiCodeAgent 已能自主多轮想 → 调工具 → 看结果地干活。但**工具执行是完全裸奔的**：工具执行链路上没有任何闸门——写文件工具能写任意路径、改文件工具能改任意文件、命令执行能跑任意命令（含 `rm -rf /`）、读写类工具能触碰项目目录之外的任何位置。这在 ch02 背景里就被列为预留能力（「在没有工具调用、**权限**、记忆等高级能力之前」）。

ch06 给 MiCodeAgent 装上**五层防御的权限系统**：危险命令黑名单硬拦截、路径沙箱限定操作范围、可配置规则做细粒度放行/拦截、多档权限模式覆盖信任等级、人在回路作为最后确认。五层串联、逐层短路，一次工具调用在真正执行前必须穿过这道护栏。权限被拒绝时把结构化拒绝**回灌**给模型让它调整策略，而非终止 Agent Loop。这是从「会自己干」到「在安全护栏内自己干」的关键一跃。
...
- **可配置规则**：以「工具名(模式)」声明 allow、ask 或 deny，支持精确匹配与 glob 匹配。
- **规则合并**：三份规则文件的规则合并为同一个集合，按 deny > ask > allow 定胜负，不做层级覆盖。
...
- F3: 规则引擎
  规则以「工具名(模式)」声明，结果只有 **allow** 或 **deny** 、ask 三种。工具名用面向用户的友好名（Bash / Read / Write / Edit / Glob / Grep），由引擎映射到对应内置工具。模式段支持**精确匹配**（如 `Bash(git status)`）与 **glob 匹配**（如 `Bash(git *)`、`Write(src/**)`）；不带模式段（如 `Bash`）表示匹配该工具的全部调用。`*` 匹配任意字符序列；`**` 的跨目录段语义**仅对文件路径有意义**，命令串不解释 `**`（等价于 `*`）。匹配对象：命令执行类比对命令串，文件类工具比对**项目相对路径**。对一次工具调用，引擎在给定规则集内判出 allow / deny / 未命中。
...
- AC5: 规则合并——三份规则文件合并为一个集合，任意来源的 deny 压过 ask、ask 压过 allow，层级不参与裁决（验证：构造跨层冲突规则断言裁决）。（F4）
