...
- [ ] 效果优先级：任意一层的 deny 压过其他层的 allow，ask 压过 allow，层级先后不影响结果（验证：引擎单测构造跨层冲突规则断言裁决）。(AC5/F4)
...
- [ ] （可选）`mypy src/micodeagent` 通过（含 `permission` 子包）。
...
- [ ] 场景 4（永久放行 + 文件产物）：对某调用选「永久」→ (a) 用 cat/grep 确认本地层配置文件出现该精确 allow 条目；(b) 重启 micodeagent 后同调用不再弹窗直接执行。(AC10/F8)
