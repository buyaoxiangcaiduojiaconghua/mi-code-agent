...
> 包名：`micodeagent`（Python 3.12+）。源码位于 `src/micodeagent/`，新增子包 `micodeagent.permission`。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/micodeagent/permission/__init__.py` | 包对外门面：导出 `Mode`/`Decision`/`Category`/`Outcome`/`Engine`/`new_engine`/`ApprovalError` 等；`Mode` 四档 + `str`/`parse_mode` |
| 新建 | `src/micodeagent/permission/blacklist.py` | 内置危险命令正则集 + `hits_blacklist`（不可配，N1） |
| 新建 | `src/micodeagent/permission/sandbox.py` | `resolve_root`、`sandbox_ok`、`eval_symlinks_or_ancestor`（N2） |
| 新建 | `src/micodeagent/permission/rule.py` | `Rule`/`RuleSet`、`parse_rule`、`match`、`match_pattern`（glob） |
| 新建 | `src/micodeagent/permission/settings.py` | `Settings` YAML、`load_settings`、`to_rule_set`、`friendly_name`、`categorize`、`extract_target` |
| 新建 | `src/micodeagent/permission/engine.py` | `Engine`、`new_engine`、`check` 前四层流水线、`mode_fallback`、`start_mode` |
| 新建 | `src/micodeagent/permission/persist.py` | `rule_for`、`persist_local_allow`（写本地层文件） |
| 新建 | `tests/test_permission_*.py` 等 | 黑名单/沙箱(含祖先回退)/规则/优先级/矩阵/加载降级/解析失败 单测 |
| 改   | `src/micodeagent/agent.py` | 删 `Mode`（迁 permission）；`Agent` 加 `engine`；`execute_batched(+mode)` 接入 `check`；`request_approval`；`ApprovalRequest` 事件；Deny 用 `ToolResult` 构造 |
| 改   | `tests/test_agent.py` | 权限集成(Allow/Deny/Ask/会话/永久)、保序回灌、只读并发不退化、取消、模式迁移 |
| 改   | `src/micodeagent/tui/app.py` | `mode`→`permission.Mode`、加 `engine`/`pending`/`approve_cursor`；`new_app` 增参；`Approving` 态分派；全局 ctrl+c/esc 覆盖 approving；`shift+tab` 循环模式(`next_mode`) |
| 改   | `src/micodeagent/tui/stream.py` | 处理 `ApprovalRequest`；`update_approving`；`submit` 保留 `/plan`·`/do`（去掉 `/mode`）；`begin_turn` 传 engine |
| 改   | `src/micodeagent/tui/view.py` | `status_bar` 左侧常驻模式（取代 provider 名）；待批准块渲染（多行三选菜单 + 光标高亮） |
| 改   | `tests/test_tui.py` | `shift+tab` 循环切换、approval 态按键回传、Esc 取消兜底、状态栏显示模式、模式跨轮保持；既有 `/plan`·`/do` 用例适配新 Mode 类型 |
| 改   | `src/micodeagent/cli.py` | 构造 `permission.new_engine(root)` 注入 `tui.new_app` |
| 改   | `smoke/main.py` | 新增 `cwd`、构造引擎、`Mode.BYPASS` 运行；`new_agent` 增参 |
| 改   | `.gitignore` | 追加 `.micodeagent/settings.local.yaml` |
| 新建 | `.micodeagent/settings.yaml.example` | 权限配置示例（default_mode + allow/deny） |

---

## T1: permission 基础类型
**文件：** `src/micodeagent/permission/__init__.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from micodeagent.permission import Mode, parse_mode; ..."` 跑通；`parse_mode` 对 `"default"/"acceptEdits"/"plan"/"bypassPermissions"`（含大小写变体）均得 `(对应档, True)`，`parse_mode("x")` 得 `(Mode.DEFAULT, False)`。

## T2: 危险命令黑名单
**文件：** `src/micodeagent/permission/blacklist.py`
**依赖：** 无
**步骤：**
...
## T3: 路径沙箱
**文件：** `src/micodeagent/permission/sandbox.py`
**依赖：** 无
**步骤：**
...
## T4: 规则与匹配
**文件：** `src/micodeagent/permission/rule.py`
**依赖：** 无
**步骤：**
...
3. `def match_pattern(pattern: str, target: str) -> bool`：`pattern == ""`→`True`；命令串整串走「命令 glob」（`*` 匹配任意字符含空格，`**` 等价 `*`）；文件路径按 `/` 分段走 `*`（段内）/`**`（跨段）（参照 `tool/glob.py` 的 `match_segments` 思路）。**实现**：将 glob 编译为正则后 `re.fullmatch`，或自实现段匹配。
...
## T5: 配置加载与映射
**文件：** `src/micodeagent/permission/settings.py`
**依赖：** T1, T4
**步骤：**
...
## T6: 引擎与前四层流水线
**文件：** `src/micodeagent/permission/engine.py`
**依赖：** T1, T2, T3, T4, T5
**步骤：**
...
2. `def new_engine(root: str) -> tuple[Engine, Exception | None]`：
   ...
   - 加载三层：user=`~/.micodeagent/settings.yaml`（`Path.home()`）、project=`<root>/.micodeagent/settings.yaml`、local=`<root>/.micodeagent/settings.local.yaml`；各 `load_settings`→`to_rule_set`；**单个文件读/解析失败仅降级跳过该文件（视为空），绝不向上抛致命异常**。
   - `local_path = <root>/.micodeagent/settings.local.yaml`。
...
**验证：** 单测：逐层短路（黑名单先于沙箱/规则；deny 规则先于模式；allow 规则不进模式）；跳层放行（非 EXEC 不被黑名单拦、Bash 不被沙箱拦）；模式矩阵逐档逐类断言（含 plan 行 Write/Exec→Ask）；效果优先级（任意一层的 deny 盖过其他层的 allow 等）；`resolve_root` 失败仍得非 None 引擎。

## T7: 会话与永久规则写入
**文件：** `src/micodeagent/permission/persist.py`
**依赖：** T5, T6
**步骤：**
...
**文件：** `src/micodeagent/agent.py`
**依赖：** T6, T7
**步骤：**
1. **模式迁移**：删除 agent 内 `Mode`/`MODE_NORMAL`/`MODE_PLAN` 定义，`from micodeagent.permission import Mode`，全部改用 `Mode.DEFAULT`/`Mode.PLAN`；`run` 形参 `mode: Mode`；`mode == Mode.PLAN` 处不变（defs 选只读、plan_reminder 注入）。
...
**验证：** `python -c "from micodeagent.agent import new_agent"` 不抛（配合 T9）；轻量自检：表驱动断言 `request_approval` 在 `asyncio.CancelledError` 抛出时正确传播、不阻塞。
...
**验证：** `python -m micodeagent` 启动可进 idle；自动化部分见 T11。
...
**文件：** `src/micodeagent/cli.py`、`smoke/main.py`、`.gitignore`、`.micodeagent/settings.yaml.example`
**依赖：** T6, T8, T10
**步骤：**
...
3. `.gitignore`：在「本地配置」段追加 `.micodeagent/settings.local.yaml`。
4. `.micodeagent/settings.yaml.example`：示例——`default_mode: default`；`permissions.allow: ["Bash(git *)", "Bash(pytest)"]`；`permissions.deny: ["Bash(rm *)", "Read(.env)", "Write(.env)"]`；注释说明三层文件与优先级，并注明**只读类默认即 Allow，allow 规则主要用于提前放行 Bash/Write，deny 规则可对只读做围栏（如 Read(.env)）**。

**验证：** `python -m micodeagent --version` 不抛；`python -m smoke` 在含 write_file 的脚本下**不阻塞、跑完**（确认 `Mode.BYPASS` 跳过 Ask）；`python -m micodeagent` 能正常启动进对话。
...
4. （可选）`mypy src/micodeagent` 通过（含 `permission` 子包）。
5. 确认 `.micodeagent/settings.local.yaml` 已被 gitignore（`git check-ignore`）；检索输出无 api_key 明文。
